...
Dummy credentials `test`/`test` work. By default, so does anything else: signatures are parsed but not verified. Set `MIRROR_S3_VALIDATE_PRESIGNED_SIGNATURES=true` to verify S3 SigV2 and SigV4 presigned query signatures plus SigV4 Authorization headers against `test`/`test` and deterministic IAM/STS credentials, including issued temporary session tokens and LocalStack gateway-port Host aliases. Signed hexadecimal payload hashes are checked; streaming request authorization is checked against its payload marker, but per-chunk signatures are not yet verified. Default listen: `http://127.0.0.1:4566`.
