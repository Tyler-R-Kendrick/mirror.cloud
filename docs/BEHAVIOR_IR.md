...
- **`rollup(list, key, value)`** builds a map from a list of records, `element[key] -> element[value]`: the map-shaped answer to a list-shaped store (Secrets Manager's `VersionIdsToStages`), which no comprehension can produce.
...
- **An operation must declare a member it addresses a resource by.** Two extracted packs did not — `workspaces` looked for a `WorkspaceId` that `StopWorkspaces` does not carry, `dms` for a `ReplicationTaskIdentifier` that `StartReplicationTask` does not carry — and in both the wrong lookup yields an empty key rather than an error, so every such call in the account writes to one shared phantom row and every call succeeds. A hand-written pack had no declaration of what it was allowed to read; a bundle does, so the loader compares `id.input_members` (and the `input.X` a `derive` reads) against the operation's input shape and refuses a mismatch. An explicit `key:` is exempt — it has said how it resolves. A transcribed defect keeps its behavior by naming the resource under the operation's `addressing:` map with the reason, which is required: the exemption exists to record a defect, not to silence the check.
- **`for_each` on a write** runs it once per element of the list it names, with the element bound to `item` — the same name a delete's `where` and a list's `filter` bind, because it means the same thing. A batch operation writes N records from one request: GuardDuty's `CreateMembers` takes `AccountDetails`, SecurityHub's `BatchImportFindings` takes `Findings`, the tagging API takes `ResourceARNList`. Six of the remaining services have an operation of this shape and none could be expressed. `key` is required alongside it: without one every element resolves the same key and the last write wins, which is the silent single-row overwrite `workspaces` and `dms` turned out to be, so the loader refuses it rather than the engine producing it.
- **`where` on a write** applies it to every stored record the predicate accepts, with the candidate bound to `item` — the same contract a delete's `where` and a list's `filter` have. It is the cascade an operation owes records nobody named in the request: deleting a Stripe customer nullifies the customer on every payment intent, charge and checkout session that references it, and the predicate is the only place that set can be described. `key` must be empty alongside it — the candidate's own key is the write's address — and the loader refuses it on a create, where there is nothing stored to select.
- **A require's `message_expr` and `fields`** evaluate the fault's message and extra envelope members as expressions, for the services whose error text names what the request asked about. Stripe answers `No such customer: 'cus_123'`, and a static message cannot carry the id; its envelope also carries a `param` member naming the offending request member, which varies per rule rather than per error-table row, so it cannot live on the error definition. `message` stays the static form; both set, the expression wins. An empty error `code` is legitimate for the same reason — Stripe's missing-param errors carry no `code` member on the wire — and the codec decides what omitting it means.
- **`spread: input`** on a write copies the request's members into the record beneath everything declared. Twenty-five packs store the request itself — `for k, v := range req.Input { rec[k] = v }` — and force a few members back on top, and enumerating those members instead does not reproduce them: the pack stores only what the request carried, so a bundle spelling out `'x' in input ? input.x : null` stores a null for every member the caller omitted and a later read answers it. Its only accepted value is `input`, because model validation has already checked the request against the generated input shape by the time an effect runs — that is what makes copying it safe, and it does not hold for anything else the write could name. Declared members win the collision, in both directions of the resource/effect order, which is what lets a bundle say *keep what the caller sent, but the id and the status are mine*.
...
2. The **equivalence gate** must pass before a pack is deleted: trace-replay of the pack's recorded (Request, Response) sequences against the engine (with token unification for divergent `Rand` consumption order), the full suite matrix re-run in engine mode (`MIRROR_ENGINE_SHADOW`), and mutation retargeting (B-IR mutants replace the deleted Go mutants).
   ...
   Two mechanisms make recordings gate what they need to:
