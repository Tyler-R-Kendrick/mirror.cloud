...
**C56. C34's cure is blocked by S3, and no provider guess can be deleted until that is faced.** #382 counted the eight demux guesses so the number can only fall. The obvious next step is to make it fall -- resolve providers from the model the way every AWS service resolves -- and the measurement says why that does not work yet.
