...
`ListMultipartUploads` reads each parameter twice -- `req.Input[name]` first, then the raw query string. `Delimiter` and `KeyMarker` are string members that restxml already binds into `Input`, so the query name is never reached and rewriting it changes nothing; reading `Input` alone passes the whole test, which is how this was settled. `MaxUploads` is the one that is *not* bound, so its query read is live -- reading `Input` alone breaks `max-uploads=2` immediately. And the third mutant flips `maxUploads < 1` to `< 0`, which differ only at zero, three lines below a `maxUploads == 0` that has already rewritten zero to 1000.

I reached the wrong conclusion twice on the way: first that the codec bound everything, then that it bound nothing. Both were guesses from reading, and both were wrong; the answer is that it binds strings and not integers, which only an experiment says.

So the repair is not "make the tests less blind" -- there is nothing for a test to see. It is to delete the dead second read for the members the codec binds, and to settle whether zero means default or InvalidArgument, and to remove the mutants along with the code they point at. Until then the suite carries three permanent failures that no amount of testing can clear, which is its own kind of dishonest green: **a mutant that cannot fail is indistinguishable, in a report, from one that has been earned.**
