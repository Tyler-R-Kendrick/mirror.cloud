...
**C18. The emulator serves differently depending on whether a gitignored directory happens to exist.** `internal/specboot` ingests `specs/<provider>/*.json` at boot when those files are on disk, and falls back to the hand-built catalog when they are not. `specs/aws/` is gitignored and is written by `make specs-sync` — which `docs/EXTENDING.md` documents as step one of adding a service. So the documented workflow leaves the tree in a state where the same commit boots a different model, and the difference is not cosmetic: `ecs.CreateCluster` resolves to `aws.ec2`, `timestream` becomes `aws.ingest.timestream`, `amplify` becomes an unknown service, and their booted-server tests fail. Delete the 96MB of fetched specs and they pass again.
