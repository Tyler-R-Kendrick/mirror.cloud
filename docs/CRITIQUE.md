...
**C31. The ratchet held, the merges went in anyway, and the baseline cannot be moved to make them legal.** Three providers -- Cloudflare KV, Hostinger DNS and the Vercel REST API -- entered as hand-written Go packs under `internal/services/`, which is the one thing `ratchet.json` exists to make unmergeable: *"New services enter as a specs/ entry plus behavior/ data, never as a Go pack."* `TestRatchetNotExceeded` said so, by name, for all three. CI on `main` went red at #328 and stayed red through #329, #330 and #331, and each of those merged on top of the failure.

The numbers the gate was reporting: **47 packs to 50**, case labels 1,734 to 1,797, services LOC **45,537 to 51,520**, fault sites 754 to 955. Measured against the extraction work it sits beside, one merge window added back more hand-written service surface than the twelve extractions before it removed.

**The obvious repair is not available, and that is the design working.** Re-cutting the baseline to accept the three packs is refused by `TestRatchetBaselineOnlyFalls`, which compares the committed file against the one on the integration branch: *"The baseline may only be lowered -- raising it defeats the migration gate,"* and *"the allowlist may only lose entries."* There is no flag and no exemption. So `TestRatchetNotExceeded` stays red on `main` and on every branch cut from it until the three packs are gone, and no amount of editing the ledger changes that.

**The root cause is one layer out, again.** These three were written in Go because nothing else was available to write them in. A bundle is validated against a generated model, models come from receivers, and mirror has receivers for Smithy and for Google Discovery -- neither of which reads an OpenAPI document. Cloudflare, Hostinger, Vercel, DigitalOcean and Hetzner all publish OpenAPI, and until something ingests it, *every* non-AWS, non-GCP provider must arrive as a hand-written pack or not arrive at all. The ratchet was never going to stop that; it can only report it, which is what it has been doing four times in a row.

So the debt is real and its repayment is specific: an OpenAPI receiver, a spec entry per provider, then three extractions that take the packs back out and lower the baseline past where it started. Nothing short of that turns this gate green, and nothing here should be read as having settled it.
