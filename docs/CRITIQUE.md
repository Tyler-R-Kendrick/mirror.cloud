...
`gcp.storage` is the finding. Its model had thirty-eight shapes and not one operation bound to any of them, because the Discovery receiver built each method's name, verb and path and never read the `request.$ref` and `response.$ref` that sit beside them; `toShape` compounded it by setting each member's shape to the member's *own name* rather than to the type it refers to, so every member of every schema dangled. So the emulator's only non-AWS provider was structurally unextractable, and nothing said so -- the same shape as the ceiling in `internal/bundled` that #301 removed, one layer further out.

**C28. The receiver was the first of two ceilings on `gcp.storage`, and the second is the codec.** The receiver now resolves refs, names each anonymous nesting for where it sits, carries path-versus-query placement and the bounds Discovery states, and synthesizes an empty response for the methods that answer with nothing: 87 operations with an input and an output shape, 360 shapes, no dangling reference. The count in C27 falls from 123 operations to 36, all of them AWS.

That is necessary and not sufficient, and the remaining half is worth stating so nobody reads "has shapes now" as "can be extracted now". `internal/proto/gcp/gcprest` routes by a hand-written `switch` over substrings of the path -- `strings.Contains(path, "/o/")` and thirteen more -- which is exactly the defect #297 removed from AWS REST routing, and it decodes by flattening the body's top-level members and the query into one map with the request path stashed under `_path`. Two consequences. Every operation now carries an `HTTP.URI` a pattern matcher could route on, and nothing reads it. And the document's `basePath` (`/storage/v1/`) is not carried anywhere: `model.Service` has no field for it, so a pattern router has no way to know which prefix of an incoming path to strip before matching `b/{bucket}`. The request shape is written to describe the flattening rather than the specification's split, for the reason this document keeps returning to -- a service validated against one description and served through another is the bug, not the tidier reading -- but that only holds while the codec keeps flattening, and nothing states the coupling except the comment on `shaper.request`.

**C29. A batch is not a list of identical things, and the schema was treating it as one.** Two gaps turned up together in `aws.config` and neither is specific to it.

`spread` accepted only the whole request. Three of Config's Put operations carry the document to store *inside* one member -- `PutConfigurationRecorder` sends a `ConfigurationRecorder`, `PutConfigRule` a `ConfigRule` -- and `PutEvaluations` sends a list of them; SecurityHub's `BatchImportFindings` is the same shape with a forty-member element. Enumerating those members instead would not reproduce the packs, for the reason `spread` exists at all: a recorder sent with no `recordingGroup` would gain a null one that a later describe would answer. It now accepts `input`, `input.<Member>` and `item`, and nothing else -- a read binding is still refused, because the point of the bound is that the value came from a request the engine validated rather than from the store. Worth stating precisely, because the original comment overstated it: validation checks the members the shape *declares*, so what the bound really guarantees is provenance, not that every key inside the value is declared.

`when` on a `for_each` write was evaluated once, before any element was bound, so it could not see `item` and could only write every element or none. Config's pack skipped the evaluations carrying no resource id, and the failure that guard prevents is not a missing row -- it is the row keyed by the empty string that every such entry in the account would share, which is C25 and C26 again from a third direction. The guard now runs per element and compiles in the element's scope. No existing bundle sets both `when` and `for_each`, so nothing changed under it.

**A hole worth looking at, recorded rather than smoothed over.** `aws.config`'s recording carries thirty superseded members across five steps -- by far the most of any recording so far. Twenty-eight of them are one divergence seen three times: the pack answered the `Evaluation` a caller pushed in, unchanged, from `GetComplianceDetailsByConfigRule`, `ListDiscoveredResources` and `GetResourceConfigHistory`, whose response shapes are `EvaluationResult`, `ResourceIdentifier` and `ConfigurationItem`. Not one of those declares `ComplianceResourceId`, `ComplianceResourceType`, `ComplianceType`, `Annotation` or `OrderingTimestamp`, so every member of every row was one no SDK could read, and the bundle projecting them into what each shape declares diverges in both directions on every member. Superseding the five steps whole would have cost two exemptions instead of thirty and thrown away the one assertion those steps still make: the number of rows and their order, which is what shows the entry carrying no resource id was skipped. The narrow form was kept for that reason, and the count is the honest measure of how little of those three operations the pack ever had to say.

**C30. The Smithy receiver drops every member's wire placement, and exactly one service is blocked by it -- but the reason it is only one is luck, not design.** `model.MemberBinding` carries where a member goes on the wire: its name, its location (label, query, header, payload), its timestamp format. Counting across the whole served model, **one service of a hundred and fifty-two carries any of it, and that one is `gcp.storage`** -- the Discovery receiver written today. All hundred and fifty-one AWS services carry none, because `internal/receiver/aws/smithy` never populates the field at all.

For four of the five AWS protocols this costs nothing visible, and that is the luck: awsJson, restJson and restXml put the member's own name in the body, and awsQuery does too, which is why four awsQuery services are already extracted and serving. The 4,187 REST operations that carry a URI have, between them, zero input members marked as a path label; routing works anyway because `httpuri` parses the pattern instead of reading the binding, which is a workaround standing in for the missing data.

ec2Query is the one that pays. It is a single service -- `aws.ec2` -- and its wire names are not its member names: `DescribeVpcsResult` declares `Vpcs` and the wire wants `vpcSet`, `RunInstances` declares `Instances` and the wire wants `instancesSet`. The pack works because it hard-codes the wire names and `awsquery.writeXML` writes whatever map keys it is handed, consulting no shape. A bundle cannot do that: the loader requires output members the shape declares, so it would emit `<Vpcs>` and no EC2 SDK could read the response. So `aws.ec2` is unextractable, and it is unextractable for a reason no amount of schema work would fix.

The fix is two-part and bounded, and it is not the schema's: teach the Smithy receiver to read `smithy.api#xmlName`, `@httpLabel`, `@httpQuery`, `@httpHeader` and `@jsonName` into `MemberBinding`, then teach `awsquery` to rename members through the output shape on the way out. Both halves need a real SDK round-trip to verify, since the whole point is what a generated client can parse.

**C30 is closed, and closing it turned up three things the entry did not know.**

*The receiver half* landed first: nine traits read into `MemberBinding` took wire names from 1,958 to 22,379 and locations from 1,084 to 9,793, and gave 2,845 of the 4,187 REST operations a declared path label where none had one. *The encoder half* is the rename this entry asked for, and it needed more than the entry described.

**First, a member's name is not the only name a shape carries, and the element inside a list is the one nobody was recording.** `Vpcs` is `vpcSet`, but the rows inside `vpcSet` are `item`, and that name lives on the *list shape's own member*, which `model.Shape` had nowhere to put -- it stored the element's target type and dropped its traits. Renaming `Vpcs` correctly and still writing `<vpcSet><member>` produces a response an EC2 client reads as an empty list. `Shape.MemberBinding` and `Shape.KeyBinding` now carry it: 1,322 lists across 21 services name their own element, and six maps name their key.

**Second, ec2Query names a member twice, and the two names are different strings.** `DryRun` answers as `dryRun` and is asked for as `DryRun`; `VpcIds` answers as `vpcSet` and is asked for as `VpcId`, flattened, with no trait recording the flattening because the protocol decides it. One `Binding.Name` cannot serve both directions, so `MemberBinding.QueryName` now carries `aws.protocols#ec2QueryName` -- 5,847 members, all in the one service that needs it. This is why ec2's pack reads the flat form keys by hand: reading them through the model found nothing.

**Third, and worst: ec2Query had no client.** Every other protocol mirror serves has a real SDK round-trip in `test/sdk/go`; the protocol whose wire names diverge furthest from its member names had none, so nothing ever asked whether a real EC2 client could read what the codec wrote. It could not, and not only for the reason C30 gave -- the pack's own `<InstanceId>` (correct only because the SDK matches element names case-insensitively) and its `<member>` wrappers were wrong on the wire the whole time. `TestAWSSDKRoundTripEC2` is that client now, and it failed on the first run against the pack this entry called working. **A protocol without a client is not verified, it is unobserved**, and that is the same shape as C27, C28 and this entry: something looks populated, carries nothing usable, and nothing says so until someone tries.

Renaming is safe in both directions because a key that is not a declared member but *is* some member's wire name passes through unchanged. The hand-written packs that already answer in wire names and the bundles that answer in declared names therefore serialize to identical bytes -- which is exactly what the extraction equivalence gate compares, so `aws.ec2` can now be extracted without the gate seeing a difference.

**And a caveat this document should carry rather than smooth over.** The claim "the other four protocols cost nothing" is checked against the *model*, which cannot show what the receiver never ingested. `@jsonName` renames a member in a JSON body exactly as `@xmlName` does in XML, and whether any served operation uses one cannot be answered from `internal/generated` -- only from the vendored specs, which are gitignored and absent from a fresh checkout. So the correct statement is narrower than it looks: ec2Query is *demonstrably* blocked, and the other protocols are *unmeasured* rather than proven clean.

**C31. The ratchet held, the merges went in anyway, and the baseline cannot be moved to make them legal.** Five providers -- Cloudflare KV, Hostinger DNS, the Vercel REST API, Azure Blobs and DigitalOcean v2 -- entered as hand-written Go packs under `internal/services/`, which is the one thing `ratchet.json` exists to make unmergeable: *"New services enter as a specs/ entry plus behavior/ data, never as a Go pack."* `TestRatchetNotExceeded` said so, by name, for every one of them.

CI on `main` went red at #328 and has stayed red through #329, #330, #331, #332 and #334, each merging on top of the failure. **Two of those five landed while this entry was being written**, which is the part worth recording: the count in a draft of this paragraph was three.

The numbers the gate is reporting: **47 packs to 52**, case labels 1,734 to 1,813, services LOC **45,537 to 51,952**, fault sites 754 to 957. The five packs are 2,061 lines between them and their tests bring the rest. Measured against the extraction work it sits beside, one merge window has added back more hand-written service surface than the twelve extractions before it removed.

**The obvious repair is not available, and that is the design working.** Re-cutting the baseline is refused by `TestRatchetBaselineOnlyFalls`, which compares the committed file against the integration branch: *"The baseline may only be lowered -- raising it defeats the migration gate,"* and *"the allowlist may only lose entries."* There is no flag and no exemption. So `TestRatchetNotExceeded` stays red on `main` and on every branch cut from it until those packs are gone, and no amount of editing the ledger changes that.

**The root cause is one layer out, again.** These five were written in Go because nothing else was available to write them in. A bundle is validated against a generated model, models come from receivers, and mirror had receivers for Smithy and for Google Discovery -- neither of which reads an OpenAPI document. Cloudflare, Hostinger, Vercel, DigitalOcean and Hetzner all publish OpenAPI, so until something ingested it, *every* non-AWS, non-GCP provider had to arrive as a hand-written pack or not arrive at all. The ratchet was never going to stop that; it can only report it, which it has now done six times running.

An OpenAPI receiver exists as of this entry, which removes the excuse but not the debt. Repayment is specific and unglamorous: a spec entry per provider, a generated model, then five extractions that take the packs back out and lower the baseline past where it started. Nothing short of that turns this gate green.

**And the failure worth naming is not the packs, it is that a red gate stopped being a stop.** A ratchet whose verdict can be merged past is a report. This document has recorded three ceilings (C27, C28, C30) that existed because nobody tried the thing a gate was describing; this is the same shape with the tools reversed -- something did say so, clearly, six times, and the saying was not connected to anything that could refuse.

**C57. C56 was wrong, and it was wrong because I measured a resolver nobody was proposing.** One commit after merging C56 I built the thing it said could not be built, and it works: seven of the eight demux guesses are gone, and every one of the 838 operation paths the nine non-AWS models declare resolves to exactly one service -- no ambiguity, no miss, nothing routed wrong.

C56's measurement was not falsified. `aws.s3` really does claim `/graphql/v2`, `POST /` really is claimed by ninety-eight services, and both numbers are reproducible. The error is in what was measured. **The question was asked over the whole bundle, and the resolver is never consulted over the whole bundle.** It runs inside a guard that was already in the demux, four lines above the eight predicates, that I had written two PRs earlier:

```go
if !awsAddressed(r) || s.resolveByModel(r) == nil {
```

A request that carries an X-Amz-Target, a SigV4 credential scope, or an AWS endpoint host has said which service it is for, and none of the eight predicates was ever reached for it. So the AWS half of the bundle is out of scope by construction, and S3's claim on `/graphql/v2` is a fact about requests that never arrive at this code. Ask the same question of the non-AWS models only and the contention is not reduced, it is *absent*: 838 paths, 838 unique answers. The four collisions that remain when AWS is included are the Fly-against-Pinpoint one C34 names, and they do not arise here for the same reason.

**The failure is a specific one and worth naming precisely: I measured the system I was describing rather than the system I would build.** The guard was mine, its comment says in so many words why an AWS-addressed request must not reach a provider predicate, and I wrote the probe as though it did not exist. What made that easy is that the probe *succeeded*: it produced a clean table, the table showed a real obstacle, and a real obstacle is much more convincing than an absent one. C56 then closed with "a future attempt that starts with the providers will get exactly as far as this one did" -- a sentence that reads as caution and functions as a discouragement, written from one measurement, about work that had not been attempted.

Two smaller things the same session got wrong and the work surfaced:

- Two of C56's probes matched **nothing** (`GET /client/v4/accounts`, `GET /bucket/key.txt`) and I treated them as data. The first is not a path Cloudflare declares -- I had truncated it -- and the second failed because that probe loaded the hand-authored catalog, where every S3 operation is bound to `POST /`. Neither was evidence about anything. A probe that reports a negative because the input was wrong looks exactly like a probe reporting a real negative, and I did not check which I had.
- The helper written to remove the guesses was called `awsService`, and `MeasureDemuxGuesses` counted it as a ninth guess. The metric was right: a function in `internal/edge` named `<provider>Service` is what it counts. That is C32 from the other side -- a generic word in a name colliding with something that reads names -- and it took a red test, not a reading, to see it.

**What the repair actually is**, recorded so the shape is reusable: the facts needed to place a provider request are ones the vendor wrote down. An OpenAPI document declares its production host in `servers` and its operations' paths in `paths`. The host was being parsed already -- `basePath` reads the same field for the version prefix -- and thrown away. Carrying it into `model.Service.Hosts` and matching the whole host (or a subdomain of it) places seven of the eight; matching paths against the operation patterns places the same seven plus Railway. `azureRequest` stays, and the model says why rather than the comment: Azure's documents declare no `servers`, and its surface is addressed by query parameter and header, so all eight of its operations are bound to `/` and there is nothing to match. It goes when `azure.blobs` becomes a bundle a receiver can read paths from.

**And the lesson that generalises past this entry.** A measurement's scope is an assumption, and it is the one part of a measurement that the numbers cannot check. C56 is the second entry in this document where confident analysis of an obstacle was cheaper than ten minutes of trying it -- C27, C28 and C30 are three ceilings that existed because nobody tried what a gate described -- and this one is worse than those, because it was not a gate reporting a limit, it was me writing one down.

**C56. C34's cure is blocked by S3, and no provider guess can be deleted until that is faced.** *(Wrong. Superseded by C57, which is one commit later and is the correction: seven of the eight guesses are gone, and the obstacle below is real only for a resolver scoped to the whole bundle, which is not the resolver this proposes. Kept as written because the error is the useful part.)* #382 counted the eight demux guesses so the number can only fall. The obvious next step is to make it fall -- resolve providers from the model the way every AWS service resolves -- and the measurement says why that does not work yet.

Every one of the eight matches a HOST **or** a PATH. Not one is host-only, so deleting any of them means the model has to answer both halves.

The host half is reachable, for five of them. Widening `serviceByLabel` from the leading host label to any whole label places `api.cloudflare.com`, `api.digitalocean.com`, `api.hetzner.cloud`, `api.hostinger.com` and `backboard.railway.com` against the endpoint prefixes the model already records, and it is strictly narrower than the `strings.Contains(host, "railway")` it would replace. Two more cannot be placed by host at all, and it is worth saying why rather than calling them exceptions: **their API hosts do not contain their names.** Fly is `api.machines.dev` and Azure Blob is `<account>.blob.core.windows.net`; nothing in either names `fly` or `azure`. A third, `vercel`, is one prefix over two services -- `vercel.api` and `vercel.kv` -- so it must fall through to something that separates them, which is what `vercelService` already does.

The path half does not work, and the reason is S3. Asking which services claim a path, over the whole bundle:

```
POST  /graphql/v2          2 service(s)  [aws.s3 railway.graphql]
GET   /v1/apps             2 service(s)  [fly.machines aws.pinpoint]
POST  /v1/apps             3 service(s)  [aws.s3 fly.machines aws.pinpoint]
GET   /v2/droplets         1 service(s)  [digitalocean.v2]
GET   /v1/servers          1 service(s)  [hetzner.v1]
POST  /                   98 service(s)  [aws.s3 aws.dynamodb aws.sqs ...]
```

`aws.s3` claims `/graphql/v2`. It claims almost anything, because path-style bucket addressing means an arbitrary first segment is a bucket name, and `POST /` is claimed by ninety-eight services besides. So a path resolver that requires ONE match declines on Railway's own endpoint, and a path resolver that takes the first match re-creates the defect this whole entry is about -- it is the same "path-style S3 fallback at the bottom took everything" that `resolveByModel`'s own doc comment records as the reason the demux was rewritten.

`GET /v1/apps` resolving to two is the honest case: that is C34's Fly-took-Pinpoint collision, and a uniqueness rule correctly refuses to pick. It is also proof the rule is the right one.

**So the order of work is the opposite of what it looked like.** The cure is not "resolve providers from the model"; it is "stop S3 claiming every path", and only then the guesses. That is a change to how one AWS service is matched, with fifty-three routing mismatches already recorded against it, and it deserves to be approached as such rather than as a step in a provider migration. Recorded here because the measurement is cheap to lose and expensive to redo, and because a future attempt that starts with the providers will get exactly as far as this one did.

**C55. The ratchet measures one directory, so moving provider code out of it reads as progress. My own fix did that, and I only noticed auditing it.** `internal/check/metrics.go` walks `internal/services` and nothing else: packs, files, LOC, case labels, fault sites and `Register` call-sites are all counted there. That is the right place to count a *pack*. It is not the whole hand-written provider surface.

The routing repair in the entry above deleted `FieldName` from `internal/services/railway/graphql` -- 23 lines and seven case labels, all counted, all genuinely gone -- and put a scanner into `internal/proto/aws/restjson`, which is not counted. Railway-specific code in that supposedly generic codec now totals **282 lines** across `railwayOp`, `railwayRoute`, `gqlRootField`, `seekFragment` and `encodeRailway`, and the file carries 23 `svc.ID == "<provider>"` branches in all. So the gate's number improved while the hand-written, provider-specific Go in the tree went UP.

**This is not an argument that the repair was wrong.** Routing belongs in the codec layer; that is where every other protocol's routing lives, and the bug being fixed was real and caller-triggerable. It is an argument that the measurement is narrower than the thing it is asked to prove, and that the difference is invisible precisely because it looks like a reduction. C41 already recorded the smell -- a branch keyed on one service id, in a file whose name mentions no provider -- and this is what that smell costs once a ratchet is pointed at a different directory: the two findings compose into a way to satisfy the gate without improving what it exists to protect.

I am recording it against my own change rather than someone else's, one entry after C54, because the failure mode is identical and the lesson only counts if it survives contact with my own work. C54's pressure was to move the baseline to the tree. This one's is subtler and does not require anyone to intend it: the honest fix and the number-improving move were the same action, so nothing felt like a compromise.

**What would actually close it** is counting provider-named symbols and `svc.ID ==` branches wherever they live, not only under `internal/services`. That is a schema change to the ratchet with its own baseline to cut and its own argument to make -- the count would jump on the day it lands, and every extraction would have to lower two numbers instead of one. It should be weighed on its merits, not slipped in behind a routing fix, which is the same reasoning C53 applied to request-shaped output. Until then this entry is the disclosure: the ratchet's numbers are a floor on the pack surface, not a measure of how much hand-written provider code the emulator contains.

**C54. The ratchet was not defeated by drift. It was defeated by being re-pinned to the drift.** `ratchet.json` records the hand-written service surface -- packs, case labels, files, LOC, fault sites, `Register` call-sites -- as a baseline that may only FALL. That is the whole anti-scope-drift design: a new Go pack cannot be merged, because the number it moves is checked against a target set before the drift. `TestRatchetNotExceeded` had been red for exactly the right reason, and `known-red.json` said so in as many words: the baseline is the PRE-DRIFT target, not a floor to raise; it goes green by deleting `azure/blobs` and `railway/graphql`, *never* by running `ratchet -write`.

A PR ran `ratchet -write`. The baseline went 47 packs to 50, 1734 case labels to 1800, 45537 LOC to 51612, 754 fault sites to 955, and `TestRatchetNotExceeded` went green -- not because the surface shrank to the target, but because the target was moved up to the surface. Its own description is candid about the mechanism: `TestRatchetBaselineOnlyFalls`, the guard that exists to stop precisely this, "fails on this PR by construction", and "merging past that one red check is the only way any ratchet gate goes green again."

**It did not even do what it set out to do, and that is the tell.** The stated goal was to get the ratchet gates green. `TestRatchetBaselineMatchesTree` stayed RED through it, because the new pin does not match the tree either -- 50 packs against 49 -- so the only gate the re-pin actually turned was the one carrying the anti-drift meaning. A re-pin that worked would have made both green; this one silenced the gate that refuses and left the bookkeeping gate complaining. That asymmetry is worth more than the intent behind it: the change traded the only real signal for nothing.

That reasoning is this document's own recurring finding pointed backwards. C50 recorded that a ratchet whose verdict can be merged past is a report rather than a gate, and the repair was to make a red mean something again -- which is what `known-red.json` did: it named the two ratchet failures as expected, with their reasons, so that the tree could keep moving while they stayed red and a NEW failure would still be distinguishable by name. The machinery to live with this red already existed and was already merged. "No branch can make progress" was not true.
...
**C53. The last pack is the one the whole design has no answer for, and saying which invariant has to give is the work.** `railway/graphql` is the final hand-written pack after `azure/blobs`, and it is not another transcription. Every other extraction moved behaviour from Go into data against a model that already existed. Railway has no model, cannot have one under the current receiver set, and breaks three assumptions the engine is built on. This entry is the analysis, written before any code, because starting blind here would produce a bundle that cannot be routed to.

**One. Routing.** Every request is `POST /graphql/v2`. The operation is chosen by substring-matching the query document -- `strings.Contains(q, "projectCreate")` -- in an order that matters, longest name first, because `project` matches `projectCreate`. `httpuri.Match` routes by method and URI and cannot tell these seven apart. This is C34's substring guess one level deeper: a field name appearing in a comment, a variable name, or a string literal misroutes, and nothing would notice.

**Two. The response shape is chosen by the CALLER, not the operation.** This is the deep one. Every protocol in this tree has an output shape that is a property of the operation: the document declares it, the bundle projects members against it, and the loader refuses a member the shape does not carry. GraphQL's selection set is part of the request, so the same field answers a different shape per call. The existing pack ignores this entirely and returns everything; a faithful emulator would project the selection set. `model.Service` has nowhere to put "the shape is a function of the request", and `output:` in a bundle is a fixed projection. Nothing in the schema as frozen expresses this.

**Three. There is no document in the tree -- but there is one upstream, and I checked this time.** `railway` appears in neither `specs/mirror.set` nor `specs/urls.tsv`, which is why specboot lists it among the services served with no generated model. GraphQL's intermediary format is the schema itself, and a receiver that turned it into `model.Service` -- root fields to operations, arguments to input members, return types to output shapes -- is the thesis applied honestly rather than a special case. So the question that decides whether that is even possible is whether Railway publishes a fetchable one, and the Upstash episode earlier in this programme is what it costs to assume rather than measure.

Measured: `POST https://backboard.railway.com/graphql/v2` answers a full introspection query **with no credentials at all** -- 656 types, 156 `Query` root fields, 244 `Mutation`, 12 `Subscription`. All seven operations the pack serves are in it, and introspection places them where the pack does not bother to: `project`, `projects` and `service` are queries, `projectCreate`, `projectDelete`, `serviceCreate` and `serviceDelete` are mutations. This is the opposite of the Upstash finding, and it removes the largest unknown in this entry.

Two concrete costs come with it, both small and both real. `scripts/specs-sync.sh` fetches with `curl -fsSL <url>`, a GET of one URL, and `specs/urls.tsv` is a three-column table with nowhere to put a request body -- introspection is a POST with a query document, so the table needs a way to express one. And narrowing is by `paths=` in `specs/mirror.set`, which means nothing to a schema with one endpoint: 656 types against seven served operations needs a root-field selector instead. Neither is a redesign; both are the kind of thing that is cheap when written down before the extraction and expensive when discovered during it.

**What is NOT a blocker, and this narrows the problem usefully.** The equivalence recording invokes a handler directly with `(Operation, Input)` and never goes through HTTP, so Railway's behaviour is recordable today, exactly as every other pack's was. The `hydrate` that flattens `variables` and a nested `input` into the top-level map is ordinary enough to express. The blockers are routing and the missing model, not the semantics.

**And `_wrap`/`_list` is back -- but this time the cure is already on the shelf.** `encodeRailway` reads two instructions the pack puts in its own output to synthesize `{data: {field: {edges: [{node}]}}}`. That is the pattern C44 predicted and C48 killed for DigitalOcean, where it died because the document declared those members as the operation's response shape. My first draft of this entry said Railway has no document to declare them, so the envelope had nowhere to go. That was wrong, and introspection says so plainly: `projects` returns `QueryProjectsConnection`, whose declared members are exactly `edges` and `pageInfo`. The envelope is not something the pack has to synthesize -- it is the response shape, and ingesting the schema retires `_wrap`/`_list` here by precisely the mechanism that retired it for DigitalOcean. Writing the claim down is what made it checkable; it survived about four minutes.

**Three ways forward, and what each costs.**

A GraphQL receiver plus a `graphql` protocol is the honest answer, and measurement moved it from the largest option to the plausible one: the document turns out to be free, so what remains is schema ingestion, a real query parser rather than substring matching, and a decision about the selection set. It is the only option that leaves the tree with no hand-written Railway anything.

A B-IR shell over a `graphql` primitive is what this project's own plan prescribes for the hard cases -- "always mechanically completable, never DynamoDB in YAML" -- and it is the cheaper honest answer: the primitive owns field extraction and selection projection, the bundle owns the seven operations' behaviour, and the primitive budget records why.

Leaving it a pack is the one option that is not available, because the ratchet counts it and would stay red forever. If that is the outcome anyone wants, it needs an explicit, justified exception in `ratchet.json` rather than a permanently red gate -- which is this document's own recurring finding, that a gate nobody can turn green stops being a gate.

**The thing to decide first is not which option, it is whether the selection set is emulated at all.** If the answer is no -- return every field the operation knows, as the pack does -- then the receiver route is tractable and the primitive may not be needed. If the answer is yes, `model.Service` grows a notion of request-shaped output that nothing else in the tree needs, and that is a schema change to weigh on its own merits rather than smuggle in behind the last extraction.

**C52. A recording could say the answer changed and not that the refusal did, and the gate was silently one-sided.** `SupersededMembers` excuses an individual member of a successful answer, with a written reason, and `Superseded` excuses a whole body. Neither could touch a fault. The comparison ran the fault branch first and returned its diffs unfiltered, so a deliberate, document-backed change to a code or a status was simply unrecordable -- the recording had to be wrong, or the bundle had to reproduce a refusal the document contradicts.

Vercel's GetProject found it. Invoked with no `idOrName`, the pack's own lookup fell through to 404; the document marks the label required, so the engine's model check answers 400 first, with this service's name for that failure. Both refuse, and they disagree about which refusal it is.

**The temptation was to delete the step**, and it is worth naming why that would have been wrong twice over. It would have removed the only assertion that the engine refuses rather than serving a project keyed on the empty string -- which the pack's fallthrough was one branch away from doing -- and it would have set the precedent that a step which fails is a step to remove. C36 is this document's record of what happens when a suite's failures become negotiable.

So `fault.code`, `fault.status` and `fault.class` are exemptible by path, through the same mechanism and the same written justification as a member. What is not exemptible, here or anywhere, is the outcome CLASS: both sides must still fault. Excusing that would let a recording say "refused" and accept an answer, which is the one thing a behavioural gate exists to catch.

The unused-exemption report moved with it, and got stricter for free. A step answers either a fault or a body, never both, so an exemption naming the wrong one is unused by construction and is now reported -- which is what stops `fault.code` from being written on a step that succeeds, where it would read as a documented refusal and gate nothing.

**C51. A recorded reference names a value by where the PACK put it, and a document can move it one level down.** `CanonicalPath` exists because a pack and its bundle can call the same member different names -- `vpcSet` against `Vpcs` -- and a read-after-create through the old name silently becomes a read of the empty string. It rewrites a path through the model's own member names. What it cannot see is nesting, because a wrapper is not a rename.

Vercel's CreateProjectEnv is the case. The pack answered the environment variable at the top level; `CreateProjectEnvResponse` declares `{created, failed}`, bulk-shaped because the real operation takes several variables at once. So the variable's id is at `id` in the recording and at `created.id` in the answer, and the step that deletes that variable by reference resolved to nothing and reported a 404.

**What makes this worth an entry is where the failure pointed.** The divergence named the delete -- a step whose bundle logic is correct and whose recording is correct -- and said `unexpected fault`. Two steps of the trace were wrong for one reason that was in neither of them. A false failure pointing at the wrong place is worse than no failure, because it is acted on: the first thing it invites is a change to the delete, which cannot work, followed by a change to the recording, which is the gate itself.

So the reference lookup crosses one declared wrapper, and only when exactly one member of the answer carries the path. Two candidates mean the reference really is ambiguous, and resolving that by luck would turn a question into a silent answer; there the lookup fails as before. It searches the ANSWER rather than the shape deliberately -- a wrapper's declared shape may be a document, as Vercel's `created` is a union of two, which says nothing about what is inside.

The same extraction found the narrower version of the same hazard and left it alone, which is worth recording too. `linkInputs` turns a literal a caller supplied into a reference when an earlier step's output happened to contain the same string: Vercel's second environment variable declares `target: ["production"]`, and the first one's DEFAULT target list contains `production`, so the literal became a reference to it. That is wrong -- it turns "the caller sent this" into "whatever the earlier step invented" -- and the obvious fix, indexing every step's supplied literals before linking any of them, breaks the mechanism outright: a recorder's own `From` references are resolved to literals before linking, so a global pre-pass would mark them supplied and bake generated identifiers into the file. The interleaving is load-bearing. Recorded here rather than fixed.
**C50a. The repair for C50, and what it deliberately does not do.** C50 records that a permitted red is a blind spot the size of every other test in the same step, and that the permitted set lived in a session's notes rather than in the tree. `known-red.json` names it: six entries, each with a reason a reader can check, and `scripts/known-red.py` reads a run and says which of its failures are on the list and which are not.

**It suppresses nothing, and that is the design rather than a limitation.** Every entry is still a failing test and still fails its step. Making the ratchet green would defeat the migration gate outright -- its whole job is to stay red until the three remaining packs are gone -- and making the equivalent S3 mutants green would hide the code they point at. What the tool changes is not what is red but what is *legible*: the end of a step now says which red, by name, instead of leaving a colour that has meant "failing" for weeks.

That is the narrowest thing that fixes the actual defect, which was never "red steps exist". It was that a red step and a redder step were indistinguishable. Both times the cost was paid at merge: once by a defect I wrote, once by a flake I did not.

Written against unfixed `main`, the first run of it printed `NEW FAILURES: TestGenerateCatalogIdempotent` above the three expected ones. That is the entire case for it, made by the thing itself.

**It got its own failure mode wrong on the first try, which is the part worth keeping.** The pattern was anchored at the start of a line. `go test` indents a subtest's result, and a log downloaded from GitHub Actions prefixes every line with a timestamp -- so run against a real CI log with twenty-four failures in it, the tool matched none of them and printed `no new failures`. A tool whose entire job is to say nothing new broke had just said that about a log it could not read.

The pattern is unanchored now, and that is the smaller half of the fix. The larger half is that the script refuses to report at all on input carrying no `go test` output -- no `ok`, no `FAIL`, no `---` line. An empty file, a run teed without `2>&1`, a pipe that sent stderr elsewhere: each produces no failures, and "no new failures" about any of them is a lie in the direction that costs the most. Both are pinned by tests, the timestamped one using the real line that was missed.

Two guards keep the list from becoming what it exists to prevent. `TestKnownRedIsDeclaredHonestly` requires every entry to carry a reason and to name a test function and a package that exist, so an entry cannot outlive the test it excuses -- the same shape as `TestMutantNeedlesExist` and `TestMakefileNamesRealPackages`, and the third time this repository has reached for it. `TestKnownRedScriptNamesANewFailure` drives the script itself, because a reporting tool that reports nothing fails silently by construction: it is only ever read when something is already wrong. And the script reports an entry it did NOT see, so a test that goes green shows up as a line to delete rather than as a permanent excuse.

**What it cannot do is judge.** A list of expected failures is an invitation to add a seventh entry rather than fix something, and nothing here prevents that; the reason field is a check a human performs, not one the tool performs. The honest defence is that adding an entry is a visible diff with a written justification, where the previous state of affairs -- a set of reds everyone had learned the shape of -- required no diff at all.
**C50. A permitted red is a blind spot the size of every other test in the same step, and I merged a failure into it twice.** `TestGenerateCatalogIdempotent` has been failing on `main` since #360. I wrote the defect in #360, merged it, and merged again past it in #361 without noticing either time.

The defect is small and worth stating first. #360 added a `paths=` selector to `specs/mirror.set` so one 10.7 MB Vercel document narrows to the operations the service serves. `narrow` treats a selector that matches nothing as fatal, deliberately and rightly -- a declaration nothing answers, producing an empty model, is this repository's recurring failure in another costume. But `mirrorgen --catalog` does not read the vendor's document at all. It reads the bootstrap catalog, a hand-written stand-in whose bindings are approximations, and whose services mostly bind every operation to `POST /` because nothing there ever needed a URI. So a selector written against `api.vercel.com`'s paths matched none of the catalog's, and the fatal error was correct about a question nobody had asked. `mirrorgen --catalog` did not narrow `vercel.api`; it exited 1 and generated nothing, for any service.

**Narrowing the catalog by the vendor's paths is a category error, and the fix is to not do it.** `applySet` now takes the mode, skips the narrowing under `--catalog`, and `TestCatalogModeIgnoresAPathSelector` pins both halves: the stand-in survives in catalog mode, and the same selector against the same stand-in is still fatal outside it, so this does not become a licence to ignore selectors generally.

**The defect took four minutes. How it survived two merges is the entry.** Both of my verifications were shaped so that a new failure could not be told from an expected one.

Locally I ran the full non-mutation suite and read it through `head -25`. The permitted reds -- two ratchet failures, each emitting nine lines, and specboot's -- fill more than twenty-five lines on their own. The truncation was not close: it could never have shown a twelfth failing package, whatever failed. In CI, the `unit and coverage floor` step *did* report `FAIL cmd/mirrorgen`, in a step that was already red for the ratchet. I read the step, saw the colour I expected, and did not read the step's log.

So the mechanism is not carelessness at one moment; it is that **"known-permitted red" is a category with no boundary.** Once a step is expected to fail, every additional failure inside it is free. C36 recorded the same structure from the other direction: a mutant scores a kill from a test that was already failing, because the harness cannot distinguish "the mutant broke this" from "this was broken before". Here a merge scores a pass from a step that was already red, for exactly the same reason, and this document has been citing C33 -- *a gate that suppresses the gates behind it reports less than no gate at all* -- all the way through the extraction while reproducing it.

**The repair is to make the permitted set explicit rather than folkloric.** Today it lives in a session's notes: three ratchet tests, one specboot count, three S3 mutants. Nothing in the tree names them, so nothing can notice when the actual failing set stops matching the expected one. A declared allow-list -- the test identifiers and why each is permitted -- turns "CI is red for the usual reasons" from a judgement into a comparison, and makes a twelfth red package a distinct, named failure rather than more of the same colour. That is the next commit after this one, and it is the same structural answer as C49's: when the slow thing cannot distinguish two cases, extract the cheap check that can.

The narrower lesson stands on its own and is cheap to keep: **never read a failure listing through `head`.** Write it to a file and grep the whole of it. The pipe that made the output readable is what made the output wrong.

**C49. A suite that enumerates what exists cannot fail on a reference to what does not, and deleting a package is the whole programme.** The `digitalocean.v2` extraction passed a full local `go test $(go list ./...)` -- 106 packages, only the permitted reds -- and then broke **three** CI steps at once. `test-snapshot`, `test-fuzz-seeds` and `test-fuzz` each hard-code `./internal/services/digitalocean/v2`, and `go test` against a path that is not there fails outright.

Both statements were true at the same time, which is what makes this worth writing down. `test-unit` runs `go list ./...`, which *derives its own inputs*: it enumerates the packages that are present, so a stale reference is invisible to it by construction. The other three targets hard-code a couple of hundred paths instead, because they select individual characterizations and fuzz targets by name and a list cannot express that. **Deleting a directory is precisely the operation that breaks the second kind and cannot break the first** -- and deleting directories is what this migration does, four times so far and three still to go.

The report gives no help either. The failing step is called "snapshots"; the log is a `go test` invocation with a hundred paths on one line and a build error at the end. Nothing names which of the four extractions in flight owns it.

So the guard is `TestMakefileNamesRealPackages`: every package path the Makefile hands to `go test` must resolve to a directory. It runs in milliseconds, names the path and the line number, and was confirmed by reintroducing the stale line and watching it fail. That is deliberately the same shape as `TestMutantNeedlesExist`, which exists because a mutant whose needle stopped matching is what a refactor breaks and what waiting for the full suite reports half an hour late. **This repository keeps discovering the same structural answer: when a slow suite is the only thing that can see a class of staleness, extract the cheap check that sees it in a second.** Two instances is a pattern; the third will be the corpus.

**C48. Deleting a codec branch is how you find out whether the model really carried it, and the last member it could not carry cost one function.** C44 predicted this one by name: `encodeDigitalOcean` synthesized `{droplet}` and `{droplets, meta}` around a bare body, from two instructions -- `_wrap` and `_list` -- that the pack put in its own output for the codec to read back. The document declares those exact members as each operation's response shape, so the prediction was that the branch is a model nobody read, and it held: the bundle projects `droplets` and `meta` by name, the generic encoder serializes them, and the branch is gone along with the route table beside it.

**One member did not fit, and it was worth the walk to find out why.** A droplet create's response is `oneOf` -- `{droplet, links}` for one name, `{droplets, links}` for several -- and the receiver exports that as a union whose members are the synthetic arm names `option0` and `option1`. Nothing on the wire distinguishes the arms; OpenAPI writes no discriminator here. So the loader, which checks every projected member against the output shape, offered the bundle a choice between two names that appear in no response, and `droplet` read as unknown.

The fix is four lines and one rule: **what a body may carry is what every arm declares**, because a union output means the body *is* one of the arms. What made it worth measuring first is that the obvious worry -- "unions are everywhere, this will move models" -- is false. Across all 154 models there are **two** union-shaped outputs in 11,644 operations: this one, and Cloudflare's KV value, which is `oneOf` a string and a blob. The second is an opaque body with no members at all, which `ScalarBody` already answered and which the new rule correctly leaves empty. One rule, two callers, no model moves. That is the same measurement C46's lesson demanded and the same answer: count what the code consumes.

**What the deletion widened is routing, which is the part a transcription can never do.** The pack's hand-written table knew eight URIs. The narrowed model declares forty, and `httpuri.Match` reaches all of them -- so `GET /v2/droplets/{id}/actions` is now an unimplemented *operation* answering 501, where before it was an unknown *path*. The difference matters to a client: one says "this emulator does not do that yet", the other says "that endpoint does not exist". A table transcribed from the eight operations someone happened to implement cannot express the distinction, and no amount of care in writing it would have.

**C47. The fuzzer found a nonsense pattern; measuring the models found the live defect it was pointing at.** `FuzzParseAndMatch` failed on a pull request that touched neither routing nor anything linked into it: `Parse("{+}/{0}").Match("0")` reported a match having bound the empty string, so a pattern naming nothing appeared to route and the label a caller would read was absent. That is a synthetic input -- no vendor writes `{+}` -- and the tempting response is to reject the nonsense and move on.

**The nonsense was a symptom.** `Match` treated a greedy label as the end of the pattern: *"it is the last thing in the pattern by construction"*, said the comment, and nothing enforced the construction. So a greedy label swallowed every remaining segment and returned, and any pattern segment after it was never compared. Scanning all 11,604 operation URIs in the generated models found **four that break the assumption, all real, all AWS**: `/v20180820/mrap/instances/{Name+}/policy`, the same with `/policystatus`, and two `{Mrap+}/routes` in S3 Control.

The consequence is not cosmetic. `GetMultiRegionAccessPointPolicy` bound `Name` to `acct/mrap-one/policy` -- the trailing literal absorbed into the value -- and **matched every request its `/policystatus` sibling was meant to answer**. Two declared operations, one reachable, chosen by whichever the ranking happened to order first. Nothing failed, because both patterns "matched".

**The first fix that came to mind would have made it worse, and the only thing that stopped it was checking.** The obvious reading of the fuzz failure is that a greedy label must be last, so a pattern where it is not should match nothing. That rule would have taken four live AWS operations off the router. What is actually correct is arithmetic: a greedy label absorbs the segments the *rest of the pattern does not need* -- `len(parts) - at - rest` -- leaving the literals after it to be compared like any others.

**Two things are worth keeping.** A fuzz finding on a degenerate input is a claim about the code's shape, not about the input: the right next question is *what real data has this property*, and here the answer was four operations that had been silently colliding. And an invariant a comment asserts "by construction" is an invariant nothing checks -- this document has now recorded that twice, with `Detect` needing its key early (C45) and a greedy label needing to be last. **The seed is committed, so the finding cannot un-find itself on a run where the fuzzer explores elsewhere.**

**C46. The receiver modelled one of the two ways OpenAPI writes a response, and the other produced a shape that looked whole and projected nothing.** A response may carry its content inline, or it may be `$ref: "#/components/responses/all_droplets"` -- a reference to a shared *response object*, which is a different thing from the schema references the receiver already handled. It modelled only the first. A referenced response decoded to a value with no content, `jsonSchema` found nothing, and the operation was handed an **empty output structure**: a model that loads, validates, and answers every projection with "not a member of".

**The incidence is not marginal, which is why this blocked a provider rather than degrading one.** DigitalOcean writes **3,731 of its 3,885 responses** that way -- 96%, every operation it has -- into 301 shared responses. Cloudflare uses 648 and Hostinger 993. So the gap was invisible for four extractions and total for the fifth.

**What made it invisible is worth separating from what made it total.** Cloudflare's KV surface is inline (0 of 28), and Hostinger's three shared responses are `ErrorResponse`, `UnauthorizedResponse` and `UnprocessableContentResponse` -- all errors, and `response()` reads only the success code and `default`, so **none of Hostinger's 993 references were ever looked at.** Repairing this moved not one byte of any committed model. A prediction that it would change two of them was wrong for an instructive reason: the count of references in a document says nothing, because only the success response is read. **Measure the thing the code actually consumes, not the thing that is easy to count.**

The repair puts responses under the rule schemas already had: follow the reference, bound the walk so a document that references itself in a circle fails the receiver instead of hanging it, and report an unresolvable one by name through the same `resolves` check -- because the alternative, still, is an empty artifact and no complaint. That is C42's shape for the fifth time in this document, and the pattern is now specific enough to state as a rule: **wherever a receiver turns absence into a default, the absence has to be distinguishable from a document that genuinely says nothing.**

**C45. Normalizing a document silently broke a property nothing had written down, and the fixture that tested it was too small to notice.** `cmd/mirrorgen` read only `.json`, so DigitalOcean -- which publishes only YAML -- could not be ingested at all. The repair looks like a one-liner: YAML and JSON are two spellings of one data model, so re-serialize YAML to JSON once, before anything asks which *specification* a document is, and every receiver gains YAML for free. Three things then went wrong, and only the third is interesting.

The first two were extension assumptions, and both announce themselves: the walk's `.json` filter, and `serviceID` trimming `.json` so that `digitalocean/v2.yaml` became the ID `digitalocean.v2.yaml`, which no line of `specs/mirror.set` names. The second was caught *by* C42's repair -- the run reported "not in ingested set" and named the service.

**The third had no such net.** `Detect` is handed the first 4096 bytes of a file, and that works for a vendor's own JSON because such a document leads with `openapi` or `smithy`. Re-serializing destroys that: `json.Marshal` sorts a map's keys, and in DigitalOcean's real document `components.examples` is a megabyte of sample payloads that sorts first. **`"openapi"` lands at byte 1,122,354.** Every receiver declined the document, the walk skipped it in silence, and `loadBundle`'s `n == 0` fallback rendered the total failure as the reassuring line *"no vendored specs; using bootstrap catalog"*.

**The synthetic fixture could not have caught it, and it passed.** A test document small enough to read is small enough that every key fits the window, so the property under test -- *the discriminating key is within the first 4096 bytes* -- is unfalsifiable at fixture scale. It was only found by running the mechanism against the actual 3 MB document it was written for, which took one command. The fix is to detect a converted document on its top-level scalars alone: a few dozen bytes whatever the document weighs, carrying exactly the keys every `Detect` reads.

**The general shape is worth naming, because this project keeps meeting it.** A normalization step changed a property that downstream code depended on and that nothing had ever stated. `Detect`'s doc comment says what it looks for; it does not say *it must appear early*, because when it was written nothing re-ordered documents. **An undocumented precondition survives exactly until something upstream stops honouring it by accident** -- and the honest defences are the two used here: assert the property at a scale where it can fail, and run the new path against the real input before believing it. This is also C42 for the fourth time, this time introduced inside the fix for it: a file in the tree, a step that skipped it, and no complaint.

**C44. A provider branch in a codec is usually a model nobody read.** C41 asked of each such branch: what in it mentions the provider? Cloudflare's two answered *nothing*, but for a reason C41 did not reach -- they were not generic rules wearing a provider's name, they were **facts the model already carried**, transcribed into Go by someone who had never generated the model.

`encodeCloudflare` synthesized `{success, errors, messages, result}` around whatever bare body the pack returned. Cloudflare's document declares exactly that object as every KV operation's response shape: `errors`, `messages` and `success` required, `result` the payload. Generate the model and the envelope is four output members a bundle projects; the encoder branch is thirty lines asserting something the shapes state. `Decode`'s other branch keyed on `svc.ID == "cloudflare.kv" && op.Name == "PutValue"` to put the raw body under `value`. The document binds a member named `body` to the payload, and types it as a union of a string and a blob -- which is the model saying *this member is the bytes*, in the same vocabulary Smithy's `@httpPayload` uses. **Twenty-two operations across nine generated models say it**, and the codec had one of them hard-coded by name.

**The distinction the generic rule has to make is the interesting part, and it is why "a payload member is the body" would have been wrong.** Both spellings live in the same protocol. Lambda's `Payload` and Glacier's `body` are blobs and mean *these bytes, untouched*; CloudFront's `DistributionConfig` and Pinpoint's `CreateApplicationRequest` are structures bound to the payload and mean *this object, serialized*. A hundred and thirty-two operations are the second kind. Reading them the same way either drops the structure or hands a pack the JSON text of itself, so the rule tests the member's **shape kind**, not its binding alone -- and both halves now carry a mutant, because a defect in either is silent for every provider that reaches it.

**Generalizing it immediately found a live defect in a service nobody was looking at, which is the argument for generalizing at all.** Lambda's `Invoke` binds `Payload` to the request body, and the pack never read it -- it rebuilt the event out of every input member that was not `FunctionName`, `InvocationType`, `LogType` or `Qualifier`. That worked only because the codec had been parsing the event as JSON and scattering its keys across the input, so an event of `{"n":7}` arrived as the input member `n`. It is a workaround for the codec, written where the codec's defect was invisible, and it fails for any event whose top-level keys collide with those four names or which is not a JSON object at all. The generic payload rule broke it loudly -- `TestBootedServerLambdaPythonInvoke` went red on the spot -- and the repair is three lines: read the member the model says is the body. **The workaround had been in place long enough to look like the design.**

**What the two branches cost is the thing worth generalizing.** They cost nothing visible, which is why they survived: the pack answered correctly, its tests passed, and the wrongness was only that the same fact existed in two places with nothing comparing them. That is the exact shape of the disagreement `adoptGenerated` found -- fifty-three bundles validated against one description of a service and served through another. **The check to apply to a codec branch is therefore not "does it name a provider" but "is it asserting a fact, and does the model already carry that fact?"** If the answer is yes, the branch is not provider-specific code, it is drift with a service id on it.

**C43. A vendor's one-document-per-platform publication is not a spec entry, it is a decision.** Cloudflare publishes its whole API as a single 24 MB OpenAPI document: 2,164 paths, 3,462 operations, 56,854 shapes once ingested, **2.8 MB gzipped as a model**. All 152 models the repo has today come to 5.6 MB together, and the largest single one is 0.3 MB. Generated whole it would be ten times the biggest model in the tree, carried by every build and every binary, to serve six operations -- the other 3,456 answering 501.

**The obvious shortcut is the one thing the pin forbids.** Trimming the document at sync time is cheap and wrong: `specs/mirror.lock` records a sha256, that hash *is* the pin, and hashing a copy we rewrote pins our rewrite rather than the vendor's bytes. `scripts/specs-sync.sh` says so in its own comments. An unannounced upstream change is supposed to be visible precisely because the committed document is the vendor's, byte for byte.

So the narrowing belongs after ingest, where it is a declared property of `specs/mirror.set` -- `paths=/client/v4/accounts/{account_id}/storage/kv/` -- rather than a silent edit to the input. The document stays vendored and hashed; the model carries what the selector reaches. For Cloudflare's KV surface that is **14 operations and 97 shapes, 6.2 KB** against 2.8 MB: the same pin, 450 times less model.

**The part worth stating generally is the failure mode it is built to refuse.** A selector that matches nothing would emit a model with no operations -- and a model with no operations is indistinguishable from a service nobody has written yet. That is C42's silence, and the embed pattern's, and the Makefile's: a declaration nothing in the tree answers, producing an empty artifact and no complaint. It is fatal here, naming the service, the prefixes, and how many operations the document actually had. **The lesson is cheap to apply forward and expensive to learn again: when you add a mechanism that selects a subset, make the empty selection loud before anything depends on it.**

**C42. `mirrorgen` ingests a document, drops it, and says nothing.** `specs/mirror.set` decides which ingested documents become models. A document under `specs/` that no line of the set declares is read, counted in "ingested N service(s)", and then discarded -- so a run reports *ingested 153* and *wrote 152*, exits 0, and names neither the service nor the reason.

I found it by accident, measuring what Cloudflare's document would generate: I dropped the 24MB file into `specs/`, ran `make generate`, and got no model and no complaint. The natural reading of that silence is that the receiver failed on the document. It had not -- the receiver was fine, the ingest was fine, and the missing piece was one line in a text file.

**The converse has always been reported.** "not in ingested set (need specs-sync)" names every service declared with no document behind it. Only the other direction was silent, and the two are different failures with different repairs: one needs a line in `specs/mirror.set`, the other needs `make specs-sync`. They are now reported separately, and the test pins that they do not merge into one list.

This is the third entry of the same shape. The embed pattern listed two providers by name, so a model generated for a third was written, committed, and not embedded. The Makefile named a package after it was deleted, so three CI steps failed before they ran a test. Here a document sits in `specs/` and the pipeline walks past it. **In each case the tree contained a file, some declaration did not mention it, and nothing said so** -- and in each case the symptom surfaced far from the cause, as "no model for <service>", as `directory not found`, as silence. The general repair is not a new gate per case; it is that every place which filters a set of files should report what it filtered out, in the same breath as what it kept.

**C41. A "provider-specific" codec branch was the generic encoder plus two lines, twice.** Hostinger's `encodeHostinger` was the generic REST/JSON encoder with one rule added: if the output carries `_list`, write that array as the whole body. Fly's machine listing is the second operation whose response shape is a list rather than an object, and writing a second branch would have made it a pattern -- three branches for one rule, in a file whose branches this migration exists to delete.

`_list` is `bir.TopLevelList`: the engine's name for an operation whose body *is* an array. It is a convention between the engine and the codec, not a fact about Hostinger, and it now lives in the generic encoder. `encodeHostinger` is gone with it; the extraction that prompted this deleted three branches rather than one.

**What made it look provider-specific was where it was first needed.** Hostinger was the first bundle with a bare-array response, so the rule arrived in Hostinger's branch, and a branch keyed on a service ID reads as being about that service whether or not it is. The check that tells the difference is cheap and worth making every time: *what in this branch mentions the provider?* For `encodeHostinger` the answer was nothing at all -- only the `if svc.ID ==` that selected it.

**C40. Two BDD tests gave themselves exactly one second to observe something that sleeps for exactly one second.** `TestSQSQueueListing`'s throttled-move-task cases start a task with `MaxNumberOfMessagesPerSecond: 1`, which makes the pack's move loop `time.Second / 1` between iterations, and the loop only writes `CANCELLED` or `FAILED` when it next wakes. The tests then polled fifty times at twenty milliseconds -- one second -- and failed if they had not seen it. They passed because the HTTP round-trips inside the poll pushed the deadline a few milliseconds past the sleep, and they lost that margin on a loaded CI runner.

**Recording it because of what it nearly cost.** It first appeared on a push that had just added a mutex to the engine, so the obvious reading was that the new lock had slowed something down. It had not, and the check is one line: `cfg.Services = []string{"aws.sqs"}`, and `registry.New` constructs only the factories whose ID is in that list. SQS is a hand-written pack, so no engine exists in that process at all and the lock cannot execute. Eight runs on `main` and six on the branch under artificial CPU load all passed locally, which is exactly what a margin-of-milliseconds test does: it declines to reproduce anywhere except where it failed.

The repair is the budget, not the assertion -- four seconds against a one-second interval, returning on the first poll that sees the status, so a fast run is still fast. **The rule worth keeping is that a test's timeout must exceed the delay it is waiting on by a margin, not tie with it.** A deadline equal to the thing it measures is not a slow test, it is a coin flip, and a coin flip in CI reads as a regression in whatever happened to be pushed.

**It recurred in the same service, in the other half of the family, and cost the same confusion.** `TestMessageMoveTaskDestinationDeletionCharacterization` starts a move task at one message per second on the real clock and then deletes the destination, and its golden recorded `moved: 0, messagesLeft: 3`. That is not a fact about the operation; it is a record of which of two goroutines the scheduler happened to run first. CI saw `moved: 1, messagesLeft: 2` and the PR it landed on touched `cmd/mirrorgen` and an OpenAPI receiver -- neither even linked into that test binary -- so again the failure pointed at whatever had just been pushed.

Eight runs on an idle machine passed. Reproducing it took deliberate contention: **eight concurrent copies of the test, three rounds, two failures in twenty-four.** So this half of the family is not a deadline that ties, it is an assertion on the *outcome* of a race, and it hides better -- a deadline can at least be read off the test, while "the delete wins" is written down nowhere.

The repair is to assert what the operation promises instead of what the scheduler did: the task fails, it names the destination, a handle was issued, and **`moved + messagesLeft == 3`** -- no message is lost. That is strictly stronger than the golden it replaces, because `moved: 0` would have passed just as happily if the emulator had dropped a message on the floor. Thirty-two runs under the same contention, no failures. **Generalized: when a test cannot control which of two concurrent things happens first, it must assert the invariant that holds either way, and a golden is the worst place to record a race because it makes the coin flip look like recorded behavior.**

**C39. The engine dropped the one guarantee every pack made without saying so, and the test that should have caught it reported a pass three times in four.** A bundle expresses uniqueness as a read and a precondition -- `reads: { taken: ... }` then `require: !taken_found` -- and the engine resolved the read, evaluated the rule, and applied the effects as three separate store operations with nothing holding them together. Sixteen concurrent creates of one name produced two or three winners: each read the absence before any of them wrote. Every hand-written pack held `p.mu` across the whole of `Invoke`, so the same sixteen produced exactly one, and `internal/chaos` asserts that per service.

**The migration removed the mutex and kept the test.** `TestHostingerConcurrentDuplicateDomains` was ported to the bundle in #345 and has been failing intermittently ever since -- it passes at `-count=1`, which is what CI runs, and fails about one run in three at `-count=5`. It passed on `main`. It passed on this branch. It failed in CI here only because a second such test, on the same defect, doubled the number of chances per run.

**A probabilistic test for a race is a report, not a gate**, which is the same sentence as C36 with the tools reversed: there the suite reported kills it had not earned, here the suite reported a guarantee it had not checked. So the replacement forces the interleaving rather than waiting for it: a store decorator holds every reader at the index lookup until its peers arrive or a short timeout passes. Without the fix it fails five runs out of five; with it, it passes five out of five. The timeout is load-bearing -- an engine that serializes its requests can only ever have one caller at the barrier, so an unbounded rendezvous would deadlock the implementation the test is asserting is correct.

The fix is the pack's own mutex, restored where the packs' behavior now lives, with two exclusions that are checkable from the bundle: an operation that `wait`s must not hold it (SQS ReceiveMessage parks until another request changes what it waits for), and a batch delegates through `Invoke` so it takes the lock per entry rather than around them all. No bundle uses `emit`, so nothing reaches another service from inside the critical section.

**What it is not is a fix to the store.** `Scope.Txn` exists and is the architecturally right home for this, but its lock is store-global and non-reentrant: running an evaluation inside one would serialize every service in the process and deadlock the moment anything reached another. A process-local per-engine mutex is exactly the guarantee the packs gave -- their own comments say "process-wide lock; per-account locks if concurrent create throughput matters" -- and it is honest about being process-local. A compare-and-set in the store is what a non-memory backend would need, and that is the entry to write when one exists.

**C38. Deleting a package left three CI steps naming a path that no longer exists, and each one failed before it ran a single test.** #345 removed `internal/services/hostinger/api`. The Makefile still named it three times: once in `test-snapshot`, once in the `test-fuzz-seeds` package list, and once in `test-fuzz` -- which also still invoked `FuzzHostingerRoute`, a fuzz target in a package that had kept the file but lost the route table it fuzzed. On `main` at 63ebff9f the *snapshots*, *fuzz seeds* and *bounded native fuzzing* steps all report `failure`, and the message is `directory not found`.

**What makes this worth an entry is how little it resembles a test failure.** `go build ./...`, `go vet ./...` and `go test ./...` are all clean, because every one of them enumerates packages and a package that is gone is simply not enumerated. Only a hand-written path disagrees with the tree, and only a target that names one. So the local signal is green, the CI signal is red, and the red is three steps deep in a job that already has a deliberately-red first step -- which is C33's shape reappearing in its milder form: not a gate suppressing other gates, but a permitted red making three unpermitted ones look like more of the same.

The general rule is the one the extraction protocol was missing: **an extraction deletes a package, and the package's name lives in more places than its imports.** `internal/allservices` is the one everybody remembers because the compiler enforces it. The Makefile, the workflow and the mutation table are not enforced by anything. The check is mechanical -- `grep -rn <pack path>` across the tree before the commit, not after CI says so -- and it is now part of the template.

Fixed here, together with the equivalent Hetzner lines, by deleting the targets rather than repointing them: a per-provider route fuzzer exists to fuzz a hand-written route table, and once the table is gone `httpuri.Match` is what routes the service, which `FuzzMatchAgainstARealService` already fuzzes against a real model.

**C37. A bundle and the pack it replaces cannot both exist, so an extraction is atomic or it is nothing.** I wrote `behavior/hostinger/api/service.yaml`, confirmed it loaded and behaved, and committed it as a *gated artifact* -- validated by CI, serving nothing, with the pack left in place until the equivalence trace was ready. There is no such state.

`TestEveryBundleAnswersOverHTTP` builds a request from each bundle's own model and sends it through the edge. For hostinger.api it asked for `DomainsGetDomainListV1` at `/api/domains/v1/portfolio` and got `501 hostinger.dns.ListDomains is not implemented` -- because the demux still routes that path to the hand-written pack, and the pack answers six invented names rather than the specification's. Two services claimed the same routes and the pack won.

Shadowing is the mechanism the repo has for "proven but not yet serving", and it does not help here for two reasons, both instructive. That gate reads `behaviors.ServiceIDs()`, the raw list, so a shadowed bundle is still asked to answer. And `TestShadowBundlesAreStillGated` refuses a shadow without an equivalence recording -- which is precisely the step that was missing, enforced by a test I had not read.

So the bundle came back out of the branch. What stays is everything that made it *possible* -- the embed patterns, the bare-array output, the `lower` helper, the vendored specification -- none of which depends on the bundle existing yet.

**The general rule is worth stating plainly, because the plan's own wording invites the mistake.** "Record the pack trace, write the bundle, prove equivalence, delete the pack" reads as four steps that can land separately. Only the first can. The middle two produce an artifact that either conflicts with the pack or is inert, and the last is the one the ratchet measures. An extraction is one commit: trace, bundle, deletion, demux, baseline. Anything less is a change that cannot be merged and has to be carried, which is how a migration stalls.

**C36. The mutation suite reported kills it had not earned, and a red test is what bought them.** A mutant is killed when the tests it names fail under the rewrite. If one of those tests is *already failing*, the harness sees a failure and reports a kill -- it cannot distinguish "the mutant broke this" from "this was broken before the mutant ran". Every mutant pointing at a red test is laundered into a green result, and the greener the report, the less anyone looks.

Five mutants name the five SQS tests C35 is about. Those tests were red for thirty commits; all five mutants reported killed for thirty commits. The moment the tests went green, `sqs-query-url-without-action-stays-sqs` survived -- and it was right to: the demux branch it rewrites routes a queue-path GET that names no action, and every request in the test that exercises it is *signed*, so `resolveByModel` reaches SQS through the credential scope whether the branch runs or not. There was no unsigned case anywhere. The mutant had never been verified, and the line it guards had no coverage at all.

**What makes this worth its own entry is the direction of the failure.** C33 was a gate that suppressed other gates; this is a gate that *reports success* from the failure of another. A suppressed gate at least shows as `skipped`. This one showed as a kill, in a suite whose entire purpose is to answer "are the tests actually looking at this", and the answer it gave was yes.

The repair is not in the harness. Running a clean baseline per mutant would double a suite that already costs twenty minutes a shard, to re-derive what the `test` job establishes once. The invariant is an ordering one and it is now written where the suite is defined: **mutation results are meaningful only on a tree whose tests already pass.** What made this reachable was not a missing check but merging past a red suite -- which is C31's sentence again, and C35's, and now this one's. Three entries, one habit.

**This is fixed as of the commit that records it, and both halves of the diagnosis were measured rather than reasoned before anything changed.** Reading `Delimiter` and `KeyMarker` from `req.Input` alone leaves the S3 and spine suites green -- their fallbacks were dead. Reading `UploadIdMarker` the same way fails `TestBootedServerS3QuerySemantics` -- its fallback was live. So the split really was whatever the hydration block happened to list, and `upload-id-marker` and `max-uploads` were simply not on it.

`ListMultipartUploads` now reads `req.Input` only; the `parameter` closure is gone; the two missing members joined the one hydration site. All three mutants are killed, and their `known-red.json` entries are deleted -- six entries to three.

The zero question is the one thing NOT settled by evidence, and it is settled by a test instead. `max-uploads=0` still folds to the default, because what the real service does with it is unknown and a guess would replace a documented unknown with an undocumented one. What killed `-accept-zero-limit` is the assertion nobody had written: a NEGATIVE limit is refused. The code had always refused one; the range check simply had no test below zero, which is why the bound could be widened and survive.

One thing the collapse exposed, worth keeping. Two shapes reach `MaxUploads` and they are not interchangeable: a request routed from HTTP carries the query's TEXT, where a non-numeric value is an `InvalidArgument` and an empty `max-uploads=` means the default; a request invoked directly carries a NUMBER. Parsing both strictly broke `TestMultipartZeroLimitsUseDefaults`, because `str` in that package is a type assertion -- `str(0)` is `""`, not `"0"` -- so an integer zero parsed as garbage and answered `InvalidArgument`.

**And the same suite fails the other way too, which took three experiments to see.** Three S3 mutants -- `s3-list-uploads-ignore-http-delimiter`, `-ignore-http-key-marker`, `-accept-zero-limit` -- have survived every run for as long as anyone has looked, and have been carried in this session's notes as a known-permitted red set. They are not blind tests. **All three are equivalent mutants: no test can kill them, because the code they rewrite has no observable effect.**

**The S3 pack reads the same query string twice, in two different places.** A block near the top of the request path hydrates `req.Input` from the URL -- `prefix`, `delimiter`, `marker`, `key-marker`, `version-id-marker` and a few more. Then `ListMultipartUploads` declares its own helper that reads `req.Input[name]` first and falls back to the raw query. For every member the first block already covered, that fallback is unreachable, so rewriting the query name it passes changes nothing. `Delimiter` and `KeyMarker` are both in the first block, which is why their mutants cannot die; reading `Input` alone for those two passes every S3 suite, which is how this was settled.

The split is not a rule about types, it is whatever that hydration block happens to list. `UploadIdMarker` is a string too, is *not* in the block, and its query read is genuinely live -- converting it breaks the pagination test at once. So is `max-uploads`. The third mutant is equivalent for an unrelated reason: it flips `maxUploads < 1` to `< 0`, and those differ only at zero, three lines below a `maxUploads == 0` that has already rewritten zero to 1000.

**Getting here took four wrong answers, and they are worth listing** because each one was a confident reading of the code rather than a measurement. That the codec bound everything; that it bound nothing; that it bound strings and not integers; and -- published in a pull request comment before it was checked -- that restxml was the thing doing the binding at all. It is not: the pack hydrates itself, and restxml never enters. Every one of those was refuted by a single experiment that took under a minute.

So the repair is not "make the tests less blind" -- there is nothing for a test to see. It is to stop reading the query twice: one hydration site, covering every member the operation needs, and no per-operation fallback behind it. Then settle whether zero means default or InvalidArgument, and delete the mutants along with the code they point at. Until then the suite carries three permanent failures that no amount of testing can clear, which is its own kind of dishonest green: **a mutant that cannot fail is indistinguishable, in a report, from one that has been earned.**

**C35. Five tests were merged without the code they test, and the repo could not tell for thirty commits.** `internal/spine`'s `TestBootedServerSQSQueryTags`, `-PathEndpointStrategy`, `-QuerySendReceiveQueueURLs`, `-Section48` and `test/behavior/aws`'s `TestSQSAdvertisedQueueURLBDD` have been red since #311 -- the commit that introduced four of them. They describe SQS's query endpoint: a queue URL that rejects account-level actions with `InvalidAction`, a `MissingParameter` fault, the `Accept: application/json` dialect, a bare `<UnknownOperationException/>` at 404, `NonExistentQueue`, `InvalidAttributeName`, `ReceiptHandleIsInvalid`. `git log -S` finds `InvalidAction` and `UnknownOperationException` in exactly one commit, in exactly one file, and that file is a test.

#311 was a restacked squash of a long branch -- its own message says *"Restacked onto current main"* -- landing 297 files and fifty-one thousand lines. The tests came across the restack and the implementation did not, which is the failure mode a squash has and a series of small commits does not: nothing in the diff distinguishes a test that describes new behavior from a test that describes behavior the restack dropped.

**What is worth recording is not the drop but the thirty commits.** C33 explains the first six: `make ratchet` failed at step three of the `test` job and skipped the unit suite entirely. But the deeper cost is what the repo did with the failures once they were visible -- this session, twice, filed them as *"pre-existing on main, undiagnosed"* and merged past them, which is C31's exact shape at a smaller scale. A known-red set that nobody owns becomes a permitted red set, and then a place to hide the next one. The first two attempts at diagnosis here also blamed the wrong commit -- once the C30 encoder rewrite, once the #309 merge -- and both were wrong; bisecting took four runs and would have taken four minutes at any point in the preceding month.

The implementation is written here, and it is mostly not new behaviour so much as behaviour that was already described elsewhere and never connected. `--advertise-url` is the plainest case: the flag is parsed, documented in `mirror --help`, carried through `config`, assigned to `edge.Server.advertise` -- and read by nothing. `spi.Request.AdvertiseURL` is a declared field that no codec ever wrote, and four packs branch on it (sqs, sns signing, cloudformation, s3 location). One line connects them, and the flag has been inert for the life of the repo.

**C34. Every new provider takes an AWS service on the way in, and the demux is built so that it must.** Three times now. Vercel took `api.ecr` and `api.iotwireless` by name, because `shortName("vercel.api")` is the generic word `api` (C32). Fly took `aws.pinpoint` by path: `flyRequest` is true of anything containing `/v1/apps`, which is also Pinpoint's GetApps. The mechanism is the same both times and it is structural.

`resolveByModel` exists because the demux used to be "a chain of about a hundred and thirty hand-written substring guesses" -- its own doc comment says so, and says why they were wrong: `looksLike(r, "es")` is true of any host containing those two letters. That chain is being rebuilt, one provider at a time. There are eight now -- fly, railway, hetzner, digitalocean, azure, cloudflare, hostinger, vercel -- each a substring guess over the host or the path, and **all eight run before the model**. So every provider added claims whatever AWS routes its paths happen to overlap, and the service that was addressed is never asked.

What makes it invisible is that the collision always lives in the *new* service. Nothing in Pinpoint changed, no Pinpoint test could fail, and the only gate that could see it is the one that asks every bundle to answer over HTTP -- which had itself been skipped by CI since #328 (C33). Two defects hid a third.

The guard here is on the class: a request that names its service the way an AWS SDK does -- a SigV4 credential scope, an X-Amz-Target, an `amazonaws.com` or `api.aws` host -- is answered from the model, and the provider guesses see only what the model could not place. Provider clients send none of those, so their own routing is untouched.

That is a guard, not a cure. **The cure is for providers to stop entering as substring guesses**, which is the same sentence as C31: they are hand-written Go because nothing else could carry them, and an OpenAPI receiver plus a spec entry gives each of them an endpoint prefix and a URI the demux can resolve from the model like every AWS service. Until then the count of guesses grows with the count of providers, and each one is a fresh chance to take a service nobody is watching.

**They are counted now, which is the least this deserved.** `demux_guesses` in `ratchet.json` pins the eight by name, so the number can only fall and a ninth provider cannot arrive quietly. This is measurement, not the cure: the branches still run, still before the model, and still match by substring. What changed is that C55's blind spot no longer covers them -- the ratchet reads one directory, `internal/services`, and these have always been in `internal/edge`, which is how 282 lines of one vendor's routing also sat unnoticed inside the AWS REST-JSON codec. The provider list is derived from the tree rather than written down, and from the union of `internal/generated` and `internal/services` because neither alone is complete: `azure` has no generated model and no `specs/mirror.set` entry -- that is precisely what makes `TestEveryServedServiceIsDescribedByASpecification` red for it -- so a list drawn from either source alone would have missed `azureRequest`, one of the eight.

**C33. The red gate was hiding nine other gates, and that is why nothing else was noticed.** The `test` job runs thirteen checks in sequence, and a failed step skips every step after it. `make ratchet` is the third. So from #328 onward, on every push to `main` and every pull request, CI ran vet, gofmt and the ratchet -- and **the extraction equivalence gate, the unit suite and coverage floor, the protocol contracts, the aws-sdk-go-v2 round-trips, the snapshots, chaos, BDD, fuzz seeds, bounded fuzzing and the race detector did not run at all.** Six merges went in that way. Only `generated-models` and the mutation shards kept reporting, because they are separate jobs.

That is the explanation for the rest of what this document has just recorded. `TestReceiveMessageWakesOnSendCharacterization` hangs for ten minutes and takes the whole SQS package with it; four `internal/spine` SQS tests fail byte-identically on `main`; three S3 mutants survive. None of it was hidden cleverly. It was hidden by a green-looking wall of *skipped*, three steps in, and the only way to see any of it was to run the suite by hand.

Each check now carries `if: ${{ !cancelled() }}`, so all of them report and the job still fails if any fails. The cost is honest and immediate: CI gets slower by the ten minutes the SQS hang burns, and a run that used to show one red step will show several. That is the point. **A gate that suppresses the gates behind it reports less than no gate at all, because the silence reads as coverage** -- and this document has now recorded, in C27, C28, C30 and C31, four separate ceilings that persisted because nobody tried the thing a gate was describing. This is the fifth, and the first where the gate was actively preventing the trying.

**C32. Naming a service after a generic word made two AWS services unreachable, and neither service changed.** The demux resolves a request by the leading label of its host, and `api.ecr.<region>.amazonaws.com` and `api.iotwireless.<region>...` both lead with `api` -- a word that names no service. `shortName("vercel.api")` is `api`, so the new provider matched the label and won the tiebreak, and `aws.api.ecr` and `aws.iotwireless` resolved to `vercel.api`.

Nothing in either AWS service's own tests could have caught it: the collision lives in a third service's *name*, and it appeared the moment that name did. `TestEveryServiceIsReachableTheWayAnSDKAddressesIt` did catch it, which is the gate earning its keep -- and it was red on `main` alongside the ratchet.

The fix, made here, is to match the whole endpoint prefix the model records before falling back to a single label, so `api.ecr` is told apart from `api` by the model rather than by luck. The general form is worth stating: **a leading host label is not a service name whenever the endpoint prefix is itself dotted**, and three services in the bundle have one -- `api.ecr`, `api.iotwireless` and `data.iot`.

### The SQS recording was the pack, and the pack was not AWS

The SQS gate went red the moment its `superseded_members` mask came off (#409), and every divergence it then reported was one where the *recording* was wrong, not the bundle. The pack answered `ApproximateReceiveCount` on every receive whether or not `AttributeNames` asked for it and never answered `MD5OfBody`; it answered an arbitrary subset of queue attributes to a `GetQueueAttributes` that named none; it dropped `SequenceNumber` from FIFO sends; it took one message per group from a FIFO receive where the reference drains a group first; it delivered a message a second time past `maxReceiveCount: 1`; and it let `StartMessageMoveTask` move messages out of a queue that was nobody's dead-letter target. Each of those is documented AWS behavior the pack never had.

The rule the previous entries built -- a recording is evidence about a pack, and is re-cut only against cited evidence -- had no mechanism behind it, so the choice was between leaving twenty-four steps superseded (a hole the size of the whole SQS surface) and editing the JSON by hand (a mirror). `recut:` is the third option: a step carries the reason and the citation, `TestRecut` replaces its expectation with what the bundle now answers, and the step is still compared in full on every run. Twenty-four SQS steps carry it. The bundle changed in two places to earn them -- `CancelMessageMoveTask` answers `ResourceNotFoundException` 404 for a handle that names no task, which is the error the operation declares, and `StartMessageMoveTask` answers `ApproximateNumberOfMessagesToMove` from a new `select.count` binding, because a selection capped by `MaxNumberOfMessagesPerSecond` could not count what it was chosen from. The SDK test that asserted the pack's message for a garbage handle now asserts the reference's.

**What this does not settle** is worth saying plainly: the reasons cite AWS's documentation, not a cassette recorded against AWS. A re-cut is only as good as its citation, and a citation to prose is weaker than one to traffic. The corpus the pipeline was designed around (§8 of BEHAVIOR_IR) is still the thing that would make these decisions checkable rather than argued.

### The catalog was a list, and the list was already `internal/generated`

`internal/catalog` was 1,961 hand-transcribed lines describing 166 services, and `specboot.adoptGenerated` overwrote every wire fact in it -- protocol, prefixes, shapes, hosts, operations -- with the generated model the moment one existed. Measured, what survived adoption was: the list of services (which `generated.ServiceIDs()` already is, plus five wire names `servedAs` already carried), three services AWS publishes no model for (22 operations), eight data-plane operations packs serve that no control-plane model describes, and three operations nothing served and AWS does not have -- `CreateTerminology`, `ListCanaries`, `DeleteNetwork`, transcription inventions that a support matrix had been advertising. `specboot.Bundle()` now derives the served model from the generated one and keeps the residue as two short tables; the catalog, its snapshot test and its 4,534-line golden are deleted.

**The finding worth recording** is what the deletion turned up: `runtime.SupportRows` and `mirror services` read `catalog.Bundle()` directly, never the adopted model, so `docs/SUPPORT.md` reported every service with the catalog's guessed protocol (awsJson1_1 for services that are restJson1 on the wire) and a mock-operation count of zero for all of them. The matrix looked authoritative and was describing a data structure nothing served. It is regenerated from the served model here -- same 166 rows, real protocols, real counts -- and the next regression of this kind cannot happen through that path because the path is gone.

Sorting the served services by ID -- the only order a derived list can honestly have -- broke ECS: `CreateCluster` answered as `aws.ec2`. The demux matched `X-Amz-Target` against each service's target prefix with a bare `HasPrefix`, so `AmazonEC2` claimed `AmazonEC2ContainerServiceV20141113.CreateCluster`, and the catalog's hand order, with ECS ahead of EC2, had been the only thing deciding it correctly. The match is now the whole segment before the dot, as `claims` already did; a mutant reverting it is killed by the ECS booted-server test. **A routing decision that a hand-written list order was silently making is the kind of thing a derivation finds**, and the reason to prefer the derivation even when the hand list happened to be right.

### The per-operation XML parser was re-deriving the model by hand

`restxml.parseXMLInput` was 787 lines: twenty-three `switch` cases, one hand-written `encoding/xml` struct per S3 operation, each renaming wire elements back to the members the pack reads -- `<AccessControlList>` to `Grants`, `<CloudFunction>` to `LambdaFunctionArn`, `<Object>` repeated to `Objects` -- and each typing booleans and counts by hand. Every one of those renamings is a fact the generated model already carries: a member's `xmlName`, whether it is an attribute, whether its list is flattened and what its elements are called. `payload.go` walks the model instead: two hundred lines for every restXml operation, not twenty-three of them.

The proof was differential rather than by eye. For each operation, a body carrying every member the model declares was decoded by both, and every key the hand parser produced had to come back from the walker with the same value; the walker may produce more, and did -- the hand structs had never modelled `BlockedEncryptionTypes`, an inventory's `SSE-KMS` key, a replication rule's `AccessControlTranslation`. The three real differences were all the hand parser's: sizes typed `int64` where counts were `int` (the pack converts either), a `TargetPrefix: ""` emitted for a member never sent, and the four named configurations, which the encoder answers back keyed by element name and which therefore keep the element-keyed walk they had.

**What the walker does that the hand parser did unevenly is validate.** `PutPublicAccessBlock` rejected an undeclared element with MalformedXML; the other twenty-two silently ignored one, because that is what `encoding/xml` does with a struct. The reference validates every configuration body against its schema. The walker does the same for all of them -- an undeclared element, a boolean that is not one, a root that is not the payload's -- and the behavior suite that asserted the one strict case passes unchanged. The twenty-seven mutation needles anchored on lines of the deleted parser became ten on the walker, one per decoding concern, each killed by a test the parser already had.

The residue is a six-entry table lifting `Objects`, `Quiet`, `LocationConstraint`, `Days`, `TagSet` and `Status` to the top of the input, because the S3 pack reads them there. It is marked as the pack's debt, and goes with the pack.

### The response encoder was a second transcription of the same model, and it was wrong in four places

After the decoder, `restxml.Encode` was still 596 lines: some twenty `if op.Name ==` blocks, each spelling an S3 response root by hand -- `<ListBucketResult>`, `<AccessControlPolicy>`, `<DeleteResult>` -- and each spelling its members' element names, wrappers and order by hand, with `write`, `writeLifecycle`, `writeFlattened` and `writeNamedConfiguration` behind them. awsQuery already had a shape-driven XML writer. It was unexported and in the wrong package, and it could not have served restXml anyway, because two facts the encoder needs were not in the model: the element a structure is written as when it is a body's root (the shape-level `xmlName`, which the receiver read for members only), and the order the specification declares members in (a map, which cannot say).

Both are in the model now. `Shape.XMLName` and `Shape.MemberOrder` are read at ingestion, `make specs-sync && make generate` at the pinned commit reproduces every model with nothing else changed (37,195 structures gain an order; four services gain roots), and `internal/proto/aws/xmlenc` is the one writer for awsQuery, ec2Query and restXml bodies, with the attribute support the ACL needed and a skip for members bound to headers. `Encode` is a root and a walk.

**Transcription had drifted in four places, and the model caught each.** `CreateMultipartUpload` answered `<CreateMultipartUploadResult>`; the reference answers `<InitiateMultipartUploadResult>`. `ListObjectVersions` answered `<ListObjectVersionsResult>`; the reference, `<ListVersionsResult>`. `ListBucketInventoryConfigurations` and `ListBucketMetricsConfigurations` answered `ListBucket…Result`; the reference drops `Bucket` from both. Every SDK read the children and never the root, which is how four wrong roots survived a suite of thirty-one encoder assertions and a hundred and thirteen S3 goldens. The suites also pinned the hand encoder's *sorted* member order in several bodies and its missing namespace on others; the reference writes declared order and the namespace, and the behavior suite already asserted the reference's order for `GetObjectAttributes` -- the one place the hand code had bothered. Those expectations follow the model now, and each is a line in this entry rather than a silent edit.

One more routing invention fell out: `GET ?object-lock` was still named `GetBucketObjectLockConfiguration`, the catalog's spelling, so its response was built without a model and the SDK read an empty configuration. It routes to `GetObjectLockConfiguration`.

The residue: `GetBucketLocation` is the S3 customization that unwraps its one member into the root (`aws.customizations#s3UnwrappedXmlOutput`, one line), and `PostObject`, which no model describes, keeps its `PostResponse` root. Twenty-nine mutation needles anchored on the deleted blocks became seven on the new path, each killed.

### Three shadows were one Go method away from serving, and each hid a cascade

`aws.kafka`, `aws.s3tables` and `aws.redshift` were complete, gated bundles that could not serve for the same reason: aws.firehose (and athena's tests) called a Go-only method on the pack -- MSK's Publish/Messages, S3 Tables' CreateTable/ApplyRows/TableRows, Redshift's CreateTable/Copy/TableRows. The shadow reasons said a primitive registry was missing; it is not needed. The bundle serves every SPI operation, and each package shrinks to the one thing no SPI operation reaches -- a topic plane, a row plane, a COPY data plane -- over collections the bundle owns. The HTTP halves of all three packs are deleted.

**Each promotion turned up a cascade the bundle had left to the pack.** Deleting an MSK cluster deleted its topic messages; deleting or renaming an S3 table deleted or moved its rows; deleting a Redshift cluster deleted its COPY tables. The bundles had skipped each one with the same reasoning -- the collection is written only by the Go-only method, so the cleanup is unobservable through the served surface -- which was true only while the pack served. The moment the bundle serves, that reasoning leaks every cluster's messages and every table's rows. Each cascade is a `delete` with a `where` now (the Go side stores the owning cluster on each record, or stores rows as an object rather than a bare array so the engine can move them), and each is guarded by a needle.
