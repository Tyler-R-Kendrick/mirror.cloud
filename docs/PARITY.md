...
| `test_sns.py::TestSNSTopicCrud::test_create_topic_test_arn` | `TestSNSControlPlaneOperations` verifies topic creation, ARN projection, attribute lookup, deletion, and post-delete absence |
| `test_sns.py::TestSNSTopicCrudV2::test_list_topic_paging` | `TestSNSListPagination` verifies the 100-item topic page and continuation token |
...
| `test_sns.py::TestSNSPlatformApplicationCrud::test_get_platform_application_attributes_non_existing_app` | `TestSNSMissingPlatformApplicationAttributes` rejects reads of a valid but missing application ARN |
| `test_sns.py::TestSNSPlatformApplicationCrud::test_set_platform_application_attributes_non_existing_app` | `TestSNSMissingPlatformApplicationAttributes` rejects updates of a valid but missing application ARN |
...
| `test_sns.py::TestSNSPlatformEndpoint::test_create_platform_endpoint_check_idempotency` | `TestSNSPlatformEndpointLifecycleValidation` returns the same endpoint for repeated token/custom-data requests and rejects conflicting attributes |
...
| `test_sns.py::TestSNSPlatformEndpointCrud::test_get_platform_endpoint_attributes_non_existent_endpoint` | `TestSNSPlatformEndpointLifecycleValidation` rejects lookup of a missing endpoint |
...
| `test_sns.py::TestSNSPlatformEndpointCrud::test_set_platform_endpoint_attributes_non_existent_endpoint` | `TestSNSSetMissingPlatformEndpointAttributes` rejects updates for a missing endpoint ARN |
...
| `test_sns.py::TestSNSPlatformEndpointCrud::test_create_platform_endpoint_custom_data` | `TestSNSPlatformEndpointAttributeValidation` preserves custom endpoint data |
...
| `test_sns.py::TestSNSTopicCrudV2::test_topic_get_attributes_with_fifo_false` | `TestSNSTopicFIFOAttributeIsImmutable` rejects changing `FifoTopic` after creation |
| `test_sns.py::TestSNSTopicCrudV2::test_add_permission_errors` | `TestSNSPermissionValidation` rejects duplicate labels and invalid actions |
...
| `test_sns.py::TestSNSTopicCrudV2::test_create_topic_in_multiple_regions` | `TestSNSPartitionARNs` verifies GovCloud topic and platform-application ARNs use the `aws-us-gov` partition |
| `test_sns.py::TestSNSPlatformEndpointCrud::test_delete_platform_endpoint_with_subscription` | `TestSNSPlatformEndpointDeletionCleansSubscriptions` removes subscriptions when their endpoint is deleted |
...
| `test_sns.py::TestSNSSubscriptionSQS::test_empty_or_wrong_message_attributes` | `TestSNSMessageAttributeValidation` rejects malformed names, types, and value combinations |
...
| `test_sns.py::TestSNSPublishCrud::test_publish_by_path_parameters` | `TestSNSTopicPublishBDD` drives the AWS Query Publish action over HTTP and verifies the XML response and SQS envelope |
| `test_sns.py::TestSNSPublishCrud::test_publish_non_existent_target` | `TestSNSPublishTargetAndSubscriptionTimingCharacterization` rejects a well-formed TargetArn for a missing platform endpoint |
...
| `test_sns.py::TestSNSSubscriptionSQS::test_publish_batch_messages_without_topic` | `TestSNSMessageStructureAndSizeValidation` rejects a batch publish to a well-formed but missing topic ARN |
...
| `test_sns.py::TestSNSSubscriptionCrudV2::test_get_subscription_attributes_error_not_exists` | `TestSNSGetMissingSubscriptionAttributes` returns NotFound for a valid-shaped but absent subscription ARN |
...
| `test_sns.py::TestSNSSubscriptionCrud::test_subscribe_idempotency` | `TestSNSSubscriptionIdempotency` preserves one ARN across equivalent raw-delivery and default-scope requests while rejecting changed attributes |
| `test_sns.py::TestSNSSubscriptionCrudV2::test_subscription_paging` | `TestSNSListSubscriptionsByTopicPagination` returns 100 subscriptions with a continuation token, then the final 20 |
...
| `test_sns.py::TestSNSSubscriptionSQS::test_redrive_policy_sqs_queue_subscription` | `TestSNSSQSSubscriptionRedrive` sends failed SQS subscription notifications to the configured DLQ |
...
| `test_sns.py::TestSNSSubscriptionSQS::test_subscription_after_failure_to_deliver` | `TestSNSSQSSubscriptionRedrive` preserves a subscription after its queue is deleted and redrives subsequent deliveries |
...
| `test_sns.py::TestSNSSubscriptionSQS::test_publish_unicode_chars` | `TestSNSUnicodeMessageToSQS` preserves non-ASCII message content through the SNS JSON envelope and SQS delivery |
| `test_sns.py::TestSNSSMS::test_subscribe_sms_endpoint` | `TestSNSSMSSubscriptionAttributes` returns the confirmed SMS subscription ARN and public attributes |
...
| `test_sns.py::TestSNSSubscriptionCrud::test_subscribe_with_invalid_protocol` | `TestSNSSubscriptionProtocolAndQueueValidation` rejects unsupported subscription protocols |
| `test_sns.py::TestSNSSubscriptionCrud::test_subscribe_with_invalid_topic` | `TestSNSSubscriptionProtocolAndQueueValidation` rejects malformed and missing topic ARNs |
| `test_sns.py::TestSNSSubscriptionCrudV2::test_subscribe_sms` | `TestSNSSubscriptionProtocolAndQueueValidation` accepts a valid E.164 SMS subscription endpoint |
| `test_sns.py::TestSNSSubscriptionCrudV2::test_subscribe_sqs_queue_url` | `TestSNSSubscriptionProtocolAndQueueValidation` rejects an SQS queue URL where an ARN is required |
| `test_sns.py::TestSNSSubscriptionCrudV2::test_subscribe_unknown_sqs_queue` | `TestSNSUnknownSQSQueueSubscription` accepts a well-formed undiscoverable queue ARN for cross-account/region use while rejecting malformed endpoints |
| `test_sns.py::TestSNSSubscriptionCrudV2::test_subscribe_unknown_topic` | `TestSNSSubscriptionProtocolAndQueueValidation` rejects a validly shaped subscription request for a missing topic |
| `test_sns.py::TestSNSSubscriptionCrudV2::test_subscribe_invalid_sms` | `TestSNSSubscriptionProtocolAndQueueValidation` rejects malformed SMS endpoints |
...
| `test_sns.py::TestSNSSubscriptionSQSFifo::test_fifo_topic_to_regular_sqs` | `TestSNSFIFOTopicToStandardSQS` allows FIFO-topic delivery to a standard queue while retaining topic-level deduplication |
| `test_sns.py::TestSNSSubscriptionSQSFifo::test_publish_fifo_messages_to_dlq` | `TestSNSFIFOPublishBatchRedrivesToFIFODLQ` preserves FIFO batch entries and redrives failed source-queue deliveries to a FIFO DLQ |
...
| `test_sns.py::TestSNSSubscriptionSQSFifo::test_publish_to_fifo_with_target_arn` | `TestSNSFIFOPublishValidationAndTopicDeduplication` verifies FIFO topics accept TargetArn aliases with structured messages and MessageGroupId |
| `test_sns.py::TestSNSSubscriptionSQSFifo::test_publish_to_fifo_topic_deduplication_on_topic_level` | `TestSNSFIFODeduplicationAcrossSubscriptions` verifies identical FIFO publishes are deduplicated once at the topic and delivered once to each subscription |
| `test_sns.py::TestSNSSubscriptionSQSFifo::test_message_to_fifo_sqs_ordering` | `TestSNSFIFOOrderingAcrossSubscriptions` verifies FIFO message order is consistent across subscribed queues |
| `test_sns.py::TestSNSSubscriptionSQSFifo::test_message_to_fifo_sqs` | `TestSNSFIFONoContentDeduplicationDelivery` verifies explicit FIFO deduplication IDs deliver once when content-based deduplication is disabled |
...
| `test_sns.py::TestSNSTopicCrudV2::test_create_topic_name_constraints` | `TestSNSTopicNameConstraints` covers the 256-character boundary, FIFO suffix coupling, and invalid names |
...
| `test_sns.py::TestSNSPublishCrud::test_topic_publish_another_region` | `TestSNSCrossRegionTopicAccessRejected` rejects cross-region Publish and PublishBatch while same-region access remains covered |
...
| `test_sns.py::TestSNSTopicCrud::test_create_duplicate_topic_with_more_tags` | `TestSNSTopicTagLifecycle` rejects duplicate creation with new tags |
| `test_sns.py::TestSNSTopicCrud::test_create_duplicate_topic_check_idempotency` | `TestSNSTopicTagLifecycle` preserves the ARN across same, subset, and empty tag re-creates |
| `test_sns.py::TestSNSTopicCrud::test_create_topic_after_delete_with_new_tags` | `TestSNSTopicTagLifecycle` permits new tags after deletion and recreation |
...
| `test_sns.py::TestSNSTopicCrud::test_delete_topic_idempotency` | `TestSNSTopicDeleteIdempotency` allows repeated deletion while keeping the topic absent |
| `test_sns.py::TestSNSTopicCrudV2::test_delete_non_existent_topic` | `TestSNSTopicDeleteIdempotency` accepts deletion of a well-formed topic ARN that was never created |
...
| `test_sns.py::TestSNSSubscriptionSQS::test_subscribe_to_sqs_with_queue_url` | `TestSNSSubscriptionProtocolAndQueueValidation` rejects queue URLs where an SQS ARN is required |
| `test_sns.py::TestSNSSubscriptionSQS::test_sqs_topic_subscription_confirmation` | `TestSNSSQSSubscriptionAutoConfirmation` verifies same-account SQS subscriptions are immediately confirmed |
...
| `test_sns.py::TestSNSSubscriptionLambda::test_sns_topic_as_lambda_dead_letter_queue` | `TestSNSLambdaFunctionDLQToTopic` creates a failing function with `DeadLetterConfig.TargetArn` set to an SNS topic, publishes through a Lambda subscription, and delivers the retries-exhausted envelope to a queue subscribed to that topic |
...
| `test_sns.py::TestSNSSubscriptionSES::test_email_sender` | `TestSNSEmailDeliveredThroughSES` sends a confirmation email through SES on subscribe and a notification after ConfirmSubscription; `GET /_aws/ses` lists the mailbox |
| `test_sns.py::TestSNSSMS::test_opt_out_phone_number_via_endpoint` | `TestSNSInternalOptOutAndRetrospectEndpoints` POSTs `/_aws/sns/phone-opt-outs` and `CheckIfPhoneNumberIsOptedOut` returns true |
...
| `test_sns.py::TestSNSCertEndpoint::test_cert_endpoint_host` | `TestSNSCertURLUsesAdvertiseHost` puts `AdvertiseURL` into `SigningCertURL`; `TestSNSCertificateEndpoint` serves both `SimpleNotificationService.pem` and LocalStack's hashed cert path |
| `test_sns.py::TestSNSRetrospectionEndpoints::test_publish_to_platform_endpoint_can_retrospect` | Direct and topic platform publishes append to `snsplat` and `GET /_aws/sns/platform-endpoint-messages` returns them |
| `test_sns.py::TestSNSRetrospectionEndpoints::test_publish_sms_can_retrospect` | `TestSNSRetrospectSMSAndPlatform` and `GET /_aws/sns/sms-messages` return stored SMS payloads filtered by phone, account, and region |
| `test_sns.py::TestSNSRetrospectionEndpoints::test_subscription_tokens_can_retrospect` | `GET /_aws/sns/subscription-tokens/{arn}` returns the pending token, 400 for a malformed ARN, and 404 for a missing ARN |
...
| `test_s3_preconditions.py::TestS3CopySourcePreconditions::test_s3_copy_object_preconditions` | `TestCopySourcePreconditionsCharacterization`, AWS SDK contract, HTTP BDD, fuzz, chaos, mutation | Mapped and green |
| `test_s3_preconditions.py::TestS3CopySourcePreconditions::test_s3_copy_object_if_source_modified_since_versioned` | Versioned characterization and contract boundary checks, HTTP BDD, fuzz, chaos, mutation | Mapped and green |
| `test_s3_preconditions.py::TestS3CopySourcePreconditions::test_s3_copy_object_if_source_unmodified_since_versioned` | Versioned characterization and contract boundary checks, HTTP BDD, fuzz, chaos, mutation | Mapped and green |
...
The ordinary local gate is green across 221 packages. The post-fix 2,602-entry mutation inventory is green in four parallel shards (all four completed successfully; 2,684.9–2,816.7s per shard). This is still a local verifier, not live AWS differential evidence.
...
| `test_sqs.py::TestSqsProvider::test_send_delay_and_wait_time` | A delayed message is absent from an immediate receive and is returned by an explicit long poll as soon as its visibility delay elapses; the receive path wakes at the earlier delayed-message boundary, with atomic Verify-style snapshot, raw HTTP BDD, and mutation coverage pinning the interaction | Mapped; focused green |
...
| `test_sqs.py::TestSqsQueryApi::test_send_message_via_queue_url_with_json_protocol` | A JSON ReceiveMessage request sent directly to the queue URL is demultiplexed to SQS and keeps the `application/x-amz-json-1.0` response contract; booted HTTP characterization pins the endpoint behavior | Mapped; focused green |
...
| `test_sqs.py::TestSQSMultiAccounts::test_cross_account_access` | A secondary account can use a primary account's QueueUrl for GetQueueAttributes, SendMessage, ReceiveMessage, DeleteMessage, and PurgeQueue; the atomic cross-account characterization pins the shared URL scope |
| `test_sqs.py::TestSQSMultiAccounts::test_cross_account_get_queue_url` | GetQueueUrl honors QueueOwnerAWSAccountId and returns the primary account's existing URL to a secondary caller; the same characterization pins owner lookup |
| `test_sqs.py::TestSQSMultiAccounts::test_delete_queue_multi_account` | Queue listings remain account-scoped: deleting one account's queue does not remove another account's queue; the atomic isolation characterization pins separate store scopes |
...
| `test_sqs.py::TestSqsProvider::test_posting_to_fifo_requires_deduplicationid_group_id` | Query SendMessage on a FIFO queue rejects missing deduplication or group identifiers with the protocol-specific InvalidParameterValue/MissingParameter faults; booted HTTP characterization pins both | Mapped; focused green |
...
| `test_sqs.py::TestSqsProvider::test_change_visibility_on_deleted_message_raises_invalid_parameter_value` | `test_change_message_visibility_on_deleted_message` already characterizes ChangeMessageVisibility of a deleted receipt handle as InvalidParameterValue | Mapped; focused green |
...
| `test_sqs.py::TestSqsQueryApi::test_get_list_queues_fails_json_format` | Query ListQueues with JSON negotiation returns the modeled ListQueuesResponse envelope; booted HTTP Section48 coverage pins the supported response shape (the duplicate upstream case is skipped) | Mapped; focused green |
