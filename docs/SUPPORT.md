...
S3 CopyObject accepts only COPY or REPLACE for x-amz-metadata-directive and x-amz-tagging-directive; omitted directives default to COPY, while unknown or lowercase values fail with InvalidArgument before creating the destination.
S3 bucket and object operations validate x-amz-expected-bucket-owner as a 12-digit account ID and return AccessDenied on owner mismatch. GetObject, HeadObject, object tagging, DeleteObject, and DeleteObjects share the same bucket-existence guard, so missing buckets return NoSuchBucket without mutation.
