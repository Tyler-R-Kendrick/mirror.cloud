...
func TestFromEnvOverridesDefault(t *testing.T) {
	...
	c := FromEnv(Default())
	if c.Bind != "0.0.0.0:9" || c.AdvertiseURL != "https://mirror.example" || c.Seed != "abc" || !c.Strict || c.PersistDir != "/data" || c.DefaultRegion != "eu-west-1" || c.DefaultAccount != "123456789012" || c.ProxyMode != "record" || c.ProxyEndpoint != "https://aws.example" {
		t.Fatalf("%+v", c)
	}
}
...
func TestFromFile(t *testing.T) {
	...
	if err := os.WriteFile(p, []byte(`{"bind":"127.0.0.1:1","advertise_url":"https://local","seed":"file","persist":"data","strict":true,"region":"west","account":"1","services":["s3"],"tiers":{"s3":"emulate"}}`), 0o644); err != nil {
		t.Fatal(err)
	}
	c := FromFile(Default(), p)
	if c.Bind != "127.0.0.1:1" || c.AdvertiseURL != "https://local" || c.Seed != "file" || c.PersistDir != "data" || !c.Strict || c.DefaultRegion != "west" || c.DefaultAccount != "1" || len(c.Services) != 1 || c.Tiers["s3"] != "emulate" {
		t.Fatalf("%+v", c)
	}
