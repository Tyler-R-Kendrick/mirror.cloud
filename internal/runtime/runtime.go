...
func SupportMatrix() string {
	...
	sb.WriteString("Optional S3 signature verification checks SigV2 and SigV4 presigned queries, Authorization headers, and browser POST policies plus single-chunk SigV4A/ECDSA presigned queries and Authorization headers for test/test and deterministic IAM/STS credentials, including issued temporary session tokens and LocalStack gateway-port Host aliases. Authorization-header timestamps enforce S3's 15-minute skew window. Signed hexadecimal payload hashes, SigV4 and SigV4A streaming chunk and trailer signature chains, and signed or unsigned SigV4 aws-chunked trailing checksums are checked. SigV4A unsigned streaming trailers remain unsupported.\n")
