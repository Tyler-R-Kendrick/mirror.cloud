...
func SupportMatrix() string {
	...
	sb.WriteString("Step Functions Task resources invoke optimized SQS SendMessage, SNS Publish, DynamoDB GetItem/PutItem/UpdateItem/DeleteItem, and nested StartExecution integrations, plus generic AWS SDK operations exposed by registered local behavior packs. Service faults use integration-specific AWS error prefixes. Standard workflows support .waitForTaskToken callbacks for optimized SQS, SNS, Lambda, and nested Step Functions with token injection, heartbeat, retry/catch, and stop invalidation; .sync service integrations are not implemented.\n")
