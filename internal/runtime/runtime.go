...
func SupportMatrix() string {
	...
	sb.WriteString("S3 DeleteObject returns LocalStack-compatible NotImplemented faults for If-Match, x-amz-if-match-size, and x-amz-if-match-last-modified-time because those directory-bucket preconditions are not implemented.\n")
	...
	sb.WriteString("CompleteMultipartUpload applies the same exact destination If-Match rule after its initiation conflict checks; rejected list conditions preserve the upload for retry.\n")
