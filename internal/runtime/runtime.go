...
var Profiles = map[string][]string{
	...
	"vercel-core":       {"vercel.api", "vercel.kv", "vercel.blob"},
	"cloudflare-core":   {"cloudflare.api"},
	"hostinger-core":    {"hostinger.dns"},
	"azure-core":        {"azure.blobs", "azure.queue", "azure.table"},
	"digitalocean-core": {"digitalocean.v2"},
	"hetzner-core":      {"hetzner.v1"},
	"railway-core":      {"railway.graphql"},
	"fly-core":          {"fly.machines"},
}
...
func SupportMatrix() string {
	...
	sb.WriteString("S3 GetObject and HeadObject support one bounded, open-ended, or suffix byte range with 206 response metadata, 416 unsatisfied-range metadata, and checksum suppression for partial data; malformed and multiple ranges fall back to a full read.\n")
	...
	sb.WriteString("S3 object and bucket tag writes enforce AWS count, uniqueness, reserved-prefix, Unicode character, and key/value length rules before persistence; tagging headers reject malformed encoding and duplicates. Bucket tag APIs require an existing bucket, and GetBucketTagging returns NoSuchTagSet when none exists.\n")
	sb.WriteString("S3 CreateBucket validates and atomically persists up to 50 tags from CreateBucketConfiguration for global and account-regional buckets; invalid tags do not reserve names, and tagged us-east-1 recreations return BucketAlreadyOwnedByYou.\n")
	sb.WriteString("S3 CreateBucket defaults ObjectOwnership to BucketOwnerEnforced, validates and atomically persists all three AWS ownership modes for global and account-regional buckets, and exposes them through SDK-compatible GetBucketOwnershipControls XML. Same-owner us-east-1 recreation remains idempotent without replacing ownership controls.\n")
	sb.WriteString("S3 PutBucketOwnershipControls accepts exactly one BucketOwnerPreferred, ObjectWriter, or BucketOwnerEnforced rule, rejects malformed replacements without changing the current rule, and returns an empty success body. DeleteBucketOwnershipControls is idempotent; subsequent gets return OwnershipControlsNotFoundError.\n")
	sb.WriteString("S3 PutPublicAccessBlock accepts only the four LocalStack-supported boolean fields, defaults omitted flags to false, rejects malformed replacements without changing state, and returns SDK-compatible XML. DeletePublicAccessBlock is idempotent; subsequent gets return NoSuchPublicAccessBlockConfiguration. Like LocalStack, the stored configuration is not enforced against ACL operations.\n")
	sb.WriteString("S3 PutBucketRequestPayment accepts Requester or BucketOwner, rejects malformed replacements without changing the current payer, and returns SDK-compatible XML; new buckets default to BucketOwner. Like LocalStack, requester-pays billing effects are not emulated.\n")
	sb.WriteString("S3 PutBucketAccelerateConfiguration accepts Enabled or Suspended, rejects malformed replacements without changing the current status, rejects bucket names containing periods, and returns SDK-compatible XML.\n")
	sb.WriteString("S3 PutBucketLogging enables or disables server-access logging, requires an existing same-account target bucket in the source Region, defaults TargetPrefix to empty, preserves grants and object-key formats, rejects malformed or invalid replacements without changing the current configuration, and returns SDK-compatible XML. Like LocalStack, target grants are stored but not validated or enforced.\n")
	sb.WriteString("S3 PutBucketCors accepts 1 to 100 rules, requires AllowedMethods and AllowedOrigins, rejects unknown fields and methods other than GET, PUT, HEAD, POST, or DELETE without replacing the current configuration, and returns SDK-compatible XML. DeleteBucketCors is idempotent; subsequent gets return NoSuchCORSConfiguration. S3 preflight and actual responses apply configured wildcard origin and header matching, method matching, credentials, exposed headers, max age, and Vary; missing or rejected preflights return AWS-compatible BadRequest or AccessForbidden faults. Without a bucket configuration, LocalStack's trusted app, localhost gateway-port, S3 website, CloudFront, and file origins receive its default CORS headers, including private-network permission when requested.\n")
	sb.WriteString("S3 PutBucketWebsite supports redirect-all or index, error, and up to 50 routing rules; it validates index suffixes, HTTP/HTTPS protocols, redirect replacement exclusivity, and empty conditions without replacing the current configuration, and returns SDK-compatible XML. S3 website virtual hosts serve GET requests with redirect-all, ordered prefix/error routing rules, root and directory indexes, custom 404 documents, object redirects, content types, ETags, and If-None-Match; missing resources and unsupported methods return LocalStack-compatible HTML. DeleteBucketWebsite is idempotent; subsequent gets return NoSuchWebsiteConfiguration.\n")
	sb.WriteString("S3 bucket notifications validate and normalize SQS, SNS, Lambda, and EventBridge configurations, verify configured destinations with LocalStack-compatible test events or Lambda dry runs, preserve valid state after rejected replacements, and apply exact-event or family-wildcard plus prefix/suffix filters. Object create, remove, tagging, ACL, and instant restore events use AWS-shaped payloads and deliver through the local SQS, SNS, Lambda, and EventBridge services; resource-policy authorization is limited to those services' existing enforcement.\n")
	sb.WriteString("S3 bucket lifecycle configuration validates and round-trips LocalStack-supported rules and transition-minimum headers, preserving prior state after rejected replacements. PutObject, PostObject, CompleteMultipartUpload, and current-object reads expose expiration headers for matching prefix, tag, combined, and size filters; like LocalStack, lifecycle rules do not delete or transition stored objects, and CopyObject does not synthesize expiration headers.\n")
	sb.WriteString("S3 analytics, inventory, metrics, and intelligent-tiering configurations use AWS REST/XML shapes and LocalStack-compatible sorted CRUD behavior. Analytics and intelligent-tiering IDs must match their request IDs; inventory validates its schema, destination ARN, format, schedule, version scope, and optional fields; metrics enforce 1,000 configurations and paginate sorted results 100 at a time.\n")
	sb.WriteString("S3 bucket and version-specific object ACLs default to private owner full control, normalize LocalStack-supported canned ACLs and grant headers, validate XML access-control policies, and round-trip through AWS REST/XML and SDK shapes. Multipart uploads retain initiation-time ACLs through completion; object ACL updates emit notification events. Current PutObjectAcl and explicit-version GetObjectAcl or PutObjectAcl requests against delete markers return MethodNotAllowed; current GetObjectAcl returns NoSuchKey. Public access block settings are stored but do not enforce ACLs, matching LocalStack.\n")
	sb.WriteString("S3 bucket policies preserve their exact JSON document, require valid JSON whose first byte is '{', reject empty objects without replacing existing state, and return raw application/json through HTTP and AWS SDK reads. DeleteBucketPolicy is idempotent; subsequent gets return NoSuchBucketPolicy.\n")
	sb.WriteString("S3 bucket encryption accepts exactly one LocalStack-supported AES256, aws:fsx, aws:backup, aws:kms, or aws:kms:dsse default rule, restricts KMSMasterKeyID to aws:kms, preserves valid state after rejected replacements, and round-trips through AWS REST/XML and SDK shapes. Defaults apply to object and multipart writes; DeleteBucketEncryption is idempotent and restores AES256 object defaults.\n")
	sb.WriteString("S3 GetBucketPolicyStatus and GetObjectTorrent return explicit MirrorNotImplemented faults because the current LocalStack provider leaves both operations unimplemented; Mirror does not fabricate successful policy analysis or torrent payloads.\n")
	sb.WriteString("S3 PutObject persists cache-control, content-disposition, content-encoding, content-language, content-type, expires, x-amz-meta-*, and website-redirect metadata per version; PutObject and metadata-replacing CopyObject normalize user metadata keys, decode RFC 2047 values before storage, and re-encode non-ASCII or control values for GetObject and HeadObject. CopyObject copies or replaces ordinary metadata according to x-amz-metadata-directive while only using an explicitly supplied destination website redirect. Missing content type defaults to binary/octet-stream. Wire responses preserve the exact ETag header name.\n")
	sb.WriteString("S3 PostObject accepts multipart browser uploads, substitutes ${filename}, persists supported form metadata, HTTP-date Expires values, selected checksums, XML tags, and server-side encryption fields through PutObject, returns 200, 201, 204, or an absolute success redirect, and validates SigV2/SigV4 field groups plus policy expiration, equality, prefix, and content-length conditions. Like LocalStack, PostObject preserves already encoded user-metadata form values while GetObject and HeadObject encode raw Unicode or control values. Optional signature validation verifies SigV2 and SigV4 policy signatures plus temporary session tokens.\n")
	sb.WriteString("S3 presigned SigV2, SigV4, and SigV4A requests enforce expiry at the deadline and return LocalStack-compatible XML AccessDenied faults with request, host, expiry, and server-time metadata. Cryptographic signature verification is disabled by default.\n")
	sb.WriteString("S3 presigned query authentication rejects incomplete SigV2, SigV4, and SigV4A parameter groups with LocalStack-compatible XML AccessDenied or AuthorizationQueryParametersError faults.\n")
	sb.WriteString("Optional S3 signature verification checks SigV2 and SigV4 presigned queries, Authorization headers, and browser POST policies plus single-chunk SigV4A/ECDSA presigned queries and Authorization headers for test/test and deterministic IAM/STS credentials, including issued temporary session tokens and LocalStack gateway-port Host aliases. SigV4 and SigV4A reject unsigned x-amz-* request headers except x-amz-content-sha256. Double-encoded or otherwise malformed SigV4 credential scopes return AuthorizationQueryParametersError before signature comparison. Authorization-header timestamps enforce S3's 15-minute skew window. Signed hexadecimal payload hashes, SigV4 and SigV4A streaming chunk and trailer signature chains, and unsigned aws-chunked trailing checksums under either authorization algorithm are checked. Malformed object aws-chunked framing and decoded-length mismatches are rejected before service invocation; cancelled multipart part streams return InternalError without storing a part and may be retried. Accepted uploads strip the transport-only aws-chunked content encoding while preserving other encodings.\n")
	sb.WriteString("S3 PutObject, CopyObject, and PostObject default to AES256 server-side encryption, honor the bucket encryption rule and explicit LocalStack-supported server-side encryption values, persist encryption metadata per object version, and expose compatible write, GetObject, and HeadObject headers. Encryption configuration and headers are emulated; stored blob bytes are not cryptographically encrypted.\n")
	sb.WriteString("S3 multipart uploads resolve and persist server-side encryption when the upload is initiated, retain it across bucket-configuration changes, and expose it from create, part upload, part copy, completion, and object-read responses.\n")
	sb.WriteString("S3 multipart uploads preserve initiation-time cache-control, content-disposition, content-encoding, content-language, content-type, expires, user metadata, and website redirects through completion.\n")
	sb.WriteString("S3 multipart uploads may omit a checksum algorithm; LocalStack-compatible create, list, completion, and ChecksumMode reads then omit checksum metadata even when an individual part supplied a checksum. Explicit CRC64NVME defaults to FULL_OBJECT and other explicit algorithms default to COMPOSITE.\n")
	sb.WriteString("S3 ListParts and ListMultipartUploads return LocalStack's webfile initiator identity; multipart upload listings also expose each upload's checksum algorithm and type.\n")
	sb.WriteString("S3 owner and canonical-grantee identities omit the deprecated DisplayName field; multipart initiators retain LocalStack's webfile DisplayName exception.\n")
	sb.WriteString("S3 PutObject validates AES256 customer-provided encryption keys, persists only the key MD5 per object version, requires matching parameters on GetObject and HeadObject, and returns SSE-C response headers. Customer keys are not persisted, and stored blob bytes are not cryptographically encrypted.\n")
	sb.WriteString("S3 multipart uploads with customer-provided encryption validate AES256 credentials at initiation and for every uploaded part, retain only the key MD5 through completion, and enforce it on final object reads.\n")
	sb.WriteString("S3 CopyObject and UploadPartCopy validate SSE-C copy-source credentials independently from destination credentials, including re-encryption with a different customer key.\n")
	sb.WriteString("S3 PutObject, CopyObject, and CreateMultipartUpload accept STANDARD, REDUCED_REDUNDANCY, STANDARD_IA, ONEZONE_IA, INTELLIGENT_TIERING, GLACIER, DEEP_ARCHIVE, GLACIER_IR, SNOW, and EXPRESS_ONEZONE storage classes. Empty values default to STANDARD; OUTPOSTS, unknown, and incorrectly cased values fail with InvalidStorageClass before creating an object or multipart upload.\n")
	sb.WriteString("S3 PutObject, CopyObject, and CreateMultipartUpload enforce the 1,024-byte UTF-8 object-key ceiling. Oversized destination keys fail with KeyTooLongError before creating an object or multipart upload.\n")
	sb.WriteString("S3 CreateBucket enforces the global bucket-name namespace across accounts and regions. Cross-account collisions return BucketAlreadyExists, same-owner collisions outside us-east-1 return BucketAlreadyOwnedByYou, and same-owner us-east-1 recreation is idempotent without replacing bucket contents.\n")
	sb.WriteString("S3 CreateBucket validates current AWS global bucket naming rules, including length, lowercase characters, endpoints, adjacent dots, dotted-decimal forms, and reserved prefixes and suffixes, before reserving global state. Account-regional namespace requests require the caller account and target Region -an suffix, use account/Region-local storage, always return a path Location, and return BucketAlreadyOwnedByYou on recreation; AWS-excluded me-central-1 and me-south-1 reject creation.\n")
	sb.WriteString("S3 CreateBucket parses and validates LocationConstraint. Regional endpoints require a matching constraint, us-east-1 rejects values outside the current AWS S3 location enum, EU stores in eu-west-1 while remaining the reported constraint, and invalid requests do not reserve bucket names. Successful us-east-1 creates report /bucket; other regions report the full global or configured endpoint URL. GetBucketLocation uses AWS's root-level XML shape.\n")
	sb.WriteString("S3 bucket-scoped operations resolve globally named buckets to their stored Region when called through another regional endpoint. All routed S3 successes and modeled faults include x-amz-request-id and x-amz-id-2; HeadBucket also returns application/xml, x-amz-access-point-alias=false, x-amz-bucket-region, and x-amz-bucket-arn. ListObjects and ListObjectsV2 return x-amz-bucket-region. Requests signed for aws-global fail with AuthorizationHeaderMalformed and the expected us-east-1 Region.\n")
	sb.WriteString("S3 UploadPart returns ETag, checksum, and encryption metadata only in headers with Content-Length 0 and no Content-Type. Successful 204 responses omit both Content-Type and Content-Length.\n")
	sb.WriteString("S3 ListBuckets enumerates caller-owned buckets across Regions, preserves creation timestamps, filters by prefix and Region, and paginates lexicographically with opaque continuation tokens and 1 through 10,000 max-buckets bounds. Parameterized responses include BucketRegion and never expose another account's buckets.\n")
	sb.WriteString("S3 CreateBucket and ListBuckets return current LocalStack-compatible bucket ARN metadata; HeadBucket returns the same ARN in x-amz-bucket-arn.\n")
	sb.WriteString("S3 DeleteObject restores the previous current body, metadata, tags, or delete marker when an explicitly selected current version is removed, and returns the deleted version and marker headers.\n")
	sb.WriteString("S3 DeleteObject with an explicit version is idempotent when the key is completely absent from a configured versioned bucket; a missing version on an existing key still returns InvalidArgument.\n")
	sb.WriteString("S3 DeleteObject treats an explicit null version as the current object in an unversioned bucket and rejects every other explicit version with InvalidArgument, including when the key is absent.\n")
	sb.WriteString("S3 DeleteObject enforces If-Match against the current object and returns LocalStack-compatible NotImplemented faults for the directory-bucket-only x-amz-if-match-size and x-amz-if-match-last-modified-time preconditions.\n")
	sb.WriteString("S3 DeleteObjects accepts 1 through 1,000 object identifiers, validates Content-MD5 or supported SDK checksum headers over the exact XML payload, decodes per-object VersionId and Quiet, deletes valid entries while reporting missing versions as NoSuchVersion, restores prior current versions, and returns AWS DeleteResult XML with verbose or quiet response semantics.\n")
	sb.WriteString("S3 Object Lock legal holds and unexpired GOVERNANCE or COMPLIANCE retention protect explicitly selected versions from DeleteObject and DeleteObjects; only GOVERNANCE retention accepts an explicit bypass, while simple deletes still create delete markers.\n")
	sb.WriteString("S3 CreateBucket Object Lock enablement automatically enables versioning for global and account-regional buckets. Configured buckets reject versioning suspension; object lock operations and governance-bypass headers reject unconfigured buckets.\n")
	sb.WriteString("S3 PutObjectLockConfiguration enables existing versioned buckets, validates Enabled plus optional GOVERNANCE or COMPLIANCE default retention with exactly one Days or Years period, and round-trips through AWS's ObjectLockConfiguration XML payload root.\n")
	sb.WriteString("S3 PutObject, CopyObject, and multipart uploads apply bucket default retention to each new version, preserve initiation-time multipart settings, accept explicit per-object retention and legal-hold overrides, reject invalid or unconfigured lock headers, and replicate version-specific lock state.\n")
	sb.WriteString("S3 GetBucketVersioning returns an empty configuration until versioning is explicitly enabled or suspended. PutBucketVersioning rejects missing and invalid states without changing the stored configuration, converts existing objects to null versions on first configuration, and makes suspended writes and deletes replace the single null version while preserving generated versions.\n")
	sb.WriteString("S3 DeleteBucket returns BucketNotEmpty while current objects, historical versions, or delete markers remain. Rejected deletion preserves bucket contents; successful deletion returns 204 after the bucket is empty.\n")
	sb.WriteString("S3 CopyObject accepts only COPY or REPLACE for x-amz-metadata-directive and x-amz-tagging-directive; omitted directives default to COPY, while unknown or lowercase values fail with InvalidArgument before creating the destination. Unchanged same-bucket/key copies fail with InvalidRequest; metadata replacement, storage class, website redirect, encryption changes, default bucket encryption, or restored sources permit them.\n")
	sb.WriteString("S3 GLACIER and DEEP_ARCHIVE objects remain visible to HeadObject but GetObject, CopyObject, and UploadPartCopy return InvalidObjectState until RestoreObject completes. Days restores complete instantly, return 202 initially and 200 when repeated, expose x-amz-restore on reads, and are isolated per object version; GLACIER_IR remains directly readable. Restore SELECT requests are not emulated, and a request without Days does not restore data.\n")
	sb.WriteString("S3 bucket and object operations validate x-amz-expected-bucket-owner as a 12-digit account ID and return AccessDenied on owner mismatch. CopyObject and UploadPartCopy additionally validate x-amz-source-expected-bucket-owner and return NoSuchBucket for missing source buckets. Every emulated operation whose AWS input supports ExpectedBucketOwner now shares the bucket-existence guard, including multipart operations, versioning, copies, multi-delete, SelectObjectContent, GetObjectTorrent, and object annotations; missing buckets return NoSuchBucket without mutation.\n")
	...
	sb.WriteString("SSM `SecureString` uses a reversible local encoding, not real encryption, and a read answers the encoded form unless it sets WithDecryption. SSM activations, sessions, inventory, patch groups, compliance and the other operations the bundle does not declare are mock tier — no SSM agent.\n")
	...
	sb.WriteString("Kinesis is a single in-memory shard per stream. DynamoDB Streams is a single shard per table; PutItem/DeleteItem/UpdateItem/BatchWrite emit records when StreamSpecification.StreamEnabled. No shard splitting or 24h retention. S3 ListObjectsV2 returns ETag, size, last-modified time, and persisted or default storage class. ListParts returns sorted, marker-paginated parts with ETag, size, millisecond timestamps, owner/initiator, checksum values, and the upload checksum algorithm/type; empty or zero pagination values use defaults, and the next marker is always zero or the last listed part, including final pages. ListMultipartUploads is key/time ordered with prefix, delimiter, URL encoding, limits, millisecond timestamps, and LocalStack-compatible paired key/upload marker pagination; mismatched pairs fail, an upload marker without a key marker is ignored, and next markers identify the last listed upload even on a final page. Multipart data operations reject missing or mismatched upload IDs and part numbers outside 1 through 10,000. S3 PutObject and CopyObject enforce destination If-Match/If-None-Match conditions; CopyObject and UploadPartCopy enforce source ETag and modification-time conditions in LocalStack's evaluation order, including ignored future If-Modified-Since values, then decode URL-encoded source keys and copy explicit source versions. Version-specific GetObject and HeadObject select the matching body, ETag, timestamp, version ID, checksum metadata, and non-STANDARD storage class; current delete markers return 404 marker headers and explicitly selected markers return 405 with their timestamp. GetObject and HeadObject partNumber reads return completed part boundaries, count, content range, and selected composite checksum, including explicit object versions, and reject mixed Range requests. UploadPartCopy shares source version and delete-marker guards and supports exact inclusive byte ranges for any nonempty source. PutObject validates explicit Content-MD5, CRC32, CRC32C, CRC64NVME, MD5, SHA1, SHA256, SHA512, XXHASH64, XXHASH3, and XXHASH128 checksums. CreateMultipartUpload validates and persists AWS's supported algorithm/type pairs without inventing a checksum when omitted, and carries its storage class and initial tags into the completed object; UploadPart calculates or validates the selected part checksum; CompleteMultipartUpload validates the selected checksum type, manifest checksums, full-object or composite value, consecutive composite parts, ordered manifest, part ETags, selected parts, non-final minimum sizes, and expected whole-object size. Supported checksums and multipart checksum types are persisted and returned by writes and ChecksumMode-enabled reads. XXHASH64, XXHASH3, and XXHASH128 use full-object checksums for single-part writes and composite checksums for multipart uploads. CompleteMultipartUpload persists multipart ETags for reads and copy conditions. GetObjectTagging and GetBucketTagging use AWS's nested TagSet/Tag REST/XML shape. S3 SelectObjectContent projects CSV/JSON columns (`s._N` or header names) with optional WHERE col = lit — not Parquet and not aggregations.\n")
	sb.WriteString("S3 composite multipart completion requires the selected checksum for every manifest part, matching LocalStack's InvalidRequest and alternate-checksum BadDigest faults.\n")
	sb.WriteString("S3 multipart completion ignores an x-amz-mp-object-size value of zero and reports LocalStack's computed-size InvalidRequest for nonzero mismatches.\n")
	sb.WriteString("S3 composite multipart completion derives its aggregate checksum from manifest part checksums and ignores a separately supplied aggregate value.\n")
	sb.WriteString("S3 multipart completion rejects an object checksum for an algorithm other than the upload's selected algorithm with LocalStack's selected-algorithm BadDigest fault.\n")
	sb.WriteString("S3 FULL_OBJECT multipart completion requires an explicit matching checksum type when the selected object checksum is supplied, matching LocalStack's BadDigest behavior.\n")
	sb.WriteString("S3 CopyObject computes and persists a requested checksum algorithm or inherits the source object's checksum algorithm, returning the checksum and FULL_OBJECT type.\n")
	...
	sb.WriteString("EC2 is control-plane records (VPC/subnet/security group/instance ids) on the ec2Query wire — no hypervisor. Extra IAM/Redshift/API Gateway/GCS ops are named control-plane records, not leftover-KV sold as emulate; RDS and SSM operations beyond those their bundles declare are mock tier.\n")
