...
func SupportMatrix() string {
	...
	sb.WriteString("Step Functions walks Pass/Succeed/Fail/Wait(instant)/Task/Choice/Parallel/Map and supports synchronous Express execution. Task Resource `lambda:function` or `states:::lambda:invoke` calls the local Lambda pack. Activity Tasks park RUNNING until GetActivityTask + SendTaskSuccess. Intrinsics, Retry/Catch, and ItemSelector are not implemented. Firehose PutRecord applies validated JSON/JSONL or base64-delimited deaggregation with the AWS 500-subrecord fallback, GZIP CloudWatch Logs decompression, metadata-free DATA_MESSAGE extraction with CONTROL_MESSAGE dropping, raw-record decompression/processing-failure envelopes, AppendDelimiterToRecord processing, and per-record synchronous Lambda transformation with bounded retries, drop handling, S3 processing-failure envelopes, and optional CloudWatch error logs before writing decoded, GZIP-compressed, or ZIP-archived bytes to the configured S3 destination using default UTC or an AWS-syntax-validated named time zone, timestamp and random-string prefix expressions with AWS success/error-prefix syntax and length validation, and AWS-style stream/version/timestamp suffixes with default or validated custom file extensions; PutRecord and PutRecordBatch validate base64 data, per-record size, batch count, and total batch size before storing anything; stream names are validated across operations, duplicate creates and missing deletes return AWS resource faults, and descriptions expose deterministic create/update timestamps; stream listing paginates alphabetically and filters by source type; tags validate, merge, selectively remove, and paginate; SSE state validates AWS-owned or customer-managed key configuration, persists across Describe, and drives Put response flags; creation requires exactly one S3 destination with valid bucket and role ARNs, S3 buffering hints validate as paired integer bounds, destination KMS configuration validates and propagates S3 SSE metadata, Direct PUT throughput hints validate and appear in source descriptions, and Kinesis-source streams validate and describe their source ARNs; destination descriptions expose stable IDs, required AWS buffering/compression/encryption defaults, and exclusive-start pagination; Extended S3 backup validates, delivers a separately configured copy, and supports one-way enablement plus nested updates; destination updates merge fields, reject stale version IDs, and increment the stream version — no caller-source authorization for decompression, metadata extraction, cryptographic at-rest transformation, Snappy/Hadoop-Snappy compression, dynamic-partition prefix expressions, MSK/database sources, Redshift, OpenSearch, Splunk, or HTTP delivery. CloudFront stores distribution/invalidation records — no CDN edge. EventBridge rules execute persistent default-bus rate/cron schedules, deliver to local SQS, SNS, Lambda, Step Functions, and API Destination targets, and persist retriable failures with bounded backoff, Retry-After timing, and standard-SQS dead-letter metadata; permanent failures go directly to the DLQ. EventBridge Scheduler executes at/rate/cron schedules in named time zones, invokes local SQS, SNS, Lambda, and asynchronous Step Functions templated targets, disperses flexible windows deterministically, and persists bounded retries with standard-SQS dead-letter delivery. Universal targets and other templated target types are not implemented.\n")
	...
	sb.WriteString("Cloud Control stores resource records — CreateResource is SUCCESS immediately, no CloudFormation handler. Serverless Application Repository stores application records — no SAR deploy. Account stores alternate contacts — no org account mutation. IoT Wireless stores destinations and devices — no LoRaWAN. S3 Tables stores table-bucket records — no Iceberg engine.\n")
