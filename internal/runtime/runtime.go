...
func SupportMatrix() string {
	...
	sb.WriteString("S3 CreateBucket parses and validates LocationConstraint. Regional endpoints require a matching constraint, us-east-1 rejects values outside the current AWS S3 location enum, EU stores in eu-west-1 while remaining the reported constraint, and invalid requests do not reserve bucket names. GetBucketLocation uses AWS's root-level XML shape.\n")
