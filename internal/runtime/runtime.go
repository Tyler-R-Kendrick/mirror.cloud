...
func SupportMatrix() string {
	...
	sb.WriteString("S3 bucket lifecycle configuration validates and round-trips LocalStack-supported rules and transition-minimum headers, preserving prior state after rejected replacements. PutObject, PostObject, and current-object reads expose expiration headers for matching prefix, tag, combined, and size filters; like LocalStack, lifecycle rules do not delete or transition stored objects, and CopyObject or multipart completion do not synthesize expiration headers.\n")
	...
	sb.WriteString("S3 ListObjects and ListObjectsV2 return SDK-compatible flattened REST/XML members and RFC 3339 timestamps, paginate lexically across objects and common prefixes, count both against MaxKeys, and honor V1 Marker/NextMarker or V2 continuation/start-after fields. ListObjects, ListObjectsV2, ListObjectVersions, and ListMultipartUploads accept only encoding-type=url and reject empty or unknown encoding methods with AWS-compatible InvalidArgument details.\n")
