...
func SupportMatrix() string {
	...
	sb.WriteString("Optional S3 signature verification checks SigV2 and SigV4 presigned queries plus SigV4 Authorization headers for test/test and deterministic IAM/STS credentials, including issued temporary session tokens and LocalStack gateway-port Host aliases. Signed hexadecimal payload hashes are checked; streaming request authorization is checked against its payload marker, but per-chunk signatures are not yet verified.\n")
