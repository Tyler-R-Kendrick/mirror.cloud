...
func SupportMatrix() string {
	...
	sb.WriteString("Kinesis is a single in-memory shard per stream. DynamoDB Streams is a single shard per table; PutItem/DeleteItem/UpdateItem/BatchWrite emit records when StreamSpecification.StreamEnabled. No shard splitting or 24h retention. S3 ListObjectsV2 returns ETag, size, last-modified time, and persisted or default storage class. S3 PutObject and CopyObject enforce destination If-Match/If-None-Match conditions; CopyObject also enforces source ETag and modification-time conditions with AWS precedence, decodes URL-encoded source keys, and copies explicit source versions. UploadPartCopy shares source version, delete-marker, ETag, and time guards and supports byte ranges for sources over 5 MiB. CompleteMultipartUpload validates the ordered caller manifest, part ETags, selected parts, and non-final minimum sizes, and persists multipart ETags for reads and copy conditions. S3 SelectObjectContent projects CSV/JSON columns (`s._N` or header names) with optional WHERE col = lit — not Parquet and not aggregations.\n")
