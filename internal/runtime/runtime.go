...
func SupportMatrix() string {
	...
	sb.WriteString("S3 PutObject, CopyObject, and PostObject default to AES256 server-side encryption, honor the first bucket encryption rule and explicit AES256, aws:kms, or aws:kms:dsse settings, persist encryption metadata per object version, and expose compatible write, GetObject, and HeadObject headers. Encryption configuration and headers are emulated; stored blob bytes are not cryptographically encrypted.\n")
