...
func SupportMatrix() string {
	...
	sb.WriteString("S3 presigned SigV2 and SigV4 requests enforce expiry at the deadline and return LocalStack-compatible XML AccessDenied faults with request, host, expiry, and server-time metadata. Cryptographic signature verification is disabled by default.\n")
	sb.WriteString("S3 presigned query authentication rejects incomplete SigV2 and SigV4 parameter groups with LocalStack-compatible XML AccessDenied or AuthorizationQueryParametersError faults.\n")
	sb.WriteString("Optional S3 signature verification checks SigV2 and SigV4 presigned queries, Authorization headers, and browser POST policies for test/test and deterministic IAM/STS credentials, including issued temporary session tokens and LocalStack gateway-port Host aliases. Authorization-header timestamps enforce S3's 15-minute skew window. Signed hexadecimal payload hashes, streaming chunk and trailer signature chains, and signed or unsigned aws-chunked trailing checksums are checked. SigV4A/ECDSA signatures remain unsupported.\n")
