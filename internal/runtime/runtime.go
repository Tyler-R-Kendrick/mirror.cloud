...
func SupportMatrix() string {
	...
	sb.WriteString("Optional S3 signature verification checks SigV2 and SigV4 presigned queries and Authorization headers for test/test and deterministic IAM/STS credentials, including issued temporary session tokens and LocalStack gateway-port Host aliases. Authorization-header timestamps enforce S3's 15-minute skew window. Signed hexadecimal payload hashes, streaming chunk and trailer signature chains, and signed or unsigned aws-chunked trailing checksums are checked. SigV4A/ECDSA signatures remain unsupported.\n")
