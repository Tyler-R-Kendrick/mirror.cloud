...
Cloud Control stores resource records — CreateResource is SUCCESS immediately, no CloudFormation handler. Serverless Application Repository stores application records — no SAR deploy. Account stores alternate contacts — no org account mutation. IoT Wireless stores destinations and devices — no LoRaWAN. S3 Tables stores table-bucket records — no Iceberg engine.
