...
func TestRecordedTraceMatchesPack(t *testing.T) {
	f, err := equivalence.LoadFile(os.DirFS("../../../equivalence"), "traces/aws.athena.json")
	if err != nil {
		t.Fatal(err)
	}
	trace := f.Trace()
	trace.Model = specboot.Bundle().ServiceByID("aws.athena")
	if trace.Model == nil {
		t.Fatal("aws.athena not in served model")
	}
