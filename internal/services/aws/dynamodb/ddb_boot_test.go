...
// The booted server imports the edge, which imports this package, so this
// test lives outside it.
func TestBootedServerDynamoDBStreams(t *testing.T) {
	...
	arn := fmt.Sprint(streams[0].(map[string]any)["StreamArn"])
	...
	iter := fmt.Sprint(it["ShardIterator"])
	if it["ShardIterator"] == nil {
