...
func TestRecordedTraceMatchesPack(t *testing.T) {
	f, err := equivalence.LoadFile(os.DirFS("../../../equivalence"), "traces/aws.kinesis.json")
	if err != nil {
		t.Fatal(err)
	}
	trace := f.Trace()
	trace.Model = specboot.Bundle().ServiceByID("aws.kinesis")
	if trace.Model == nil {
		t.Fatal("aws.kinesis not in served model")
	}
