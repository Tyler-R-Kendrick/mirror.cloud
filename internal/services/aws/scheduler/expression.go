...
func parseScheduleExpression(raw, timezone string) (scheduleExpression, error) {
	...
	if timezone != "" {
		...
		if err != nil {
			return scheduleExpression{}, fmt.Errorf("invalid timezone %q", timezone)
		}
	}
	...
	if value, ok := body("at"); ok {
		...
		if err != nil {
			return scheduleExpression{}, fmt.Errorf("invalid at expression")
		}
		return scheduleExpression{at: at}, nil
	}
	if value, ok := body("rate"); ok {
		parts := strings.Fields(value)
		if len(parts) != 2 {
			return scheduleExpression{}, fmt.Errorf("invalid rate expression")
		}
		n, err := strconv.ParseInt(parts[0], 10, 64)
		if err != nil || n < 1 {
			return scheduleExpression{}, fmt.Errorf("invalid rate value")
		}
		...
		if !ok || (n == 1) != !strings.HasSuffix(parts[1], "s") {
			return scheduleExpression{}, fmt.Errorf("invalid rate unit")
		}
		if n > int64((1<<63-1)/unit) {
			return scheduleExpression{}, fmt.Errorf("rate value is too large")
		}
		return scheduleExpression{every: time.Duration(n) * unit}, nil
	}
	if value, ok := body("cron"); ok {
		cron, err := parseCron(value, loc)
		return scheduleExpression{cron: cron}, err
	}
	return scheduleExpression{}, fmt.Errorf("invalid schedule expression")
}
...
func (e scheduleExpression) after(t time.Time) time.Time {
	switch {
	case e.every > 0:
