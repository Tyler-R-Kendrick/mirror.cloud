...
func TestSNSCrossRegionTopicAccessRejected(t *testing.T) {
	...
	p := New(deps)
	owner := spi.Identity{Account: "111111111111", Region: "us-east-1"}
	caller := spi.Identity{Account: "222222222222", Region: "us-west-2"}
...
func TestSNSControlPlaneOperations(t *testing.T) {
	...
	if subAttributes["RawMessageDelivery"] != "true" || subAttributes["ConfirmationWasAuthenticated"] != "true" || subAttributes["Owner"] != id.Account {
...
func TestSNSFIFOOrderingAcrossSubscriptions(t *testing.T) {
	...
	id := spi.Identity{Account: "1", Region: "us-east-1"}
	topic := str(invokeSNS(t, p, id, "CreateTopic", map[string]any{
		"Name": "topic-order.fifo", "Attributes": map[string]any{"FifoTopic": "true", "ContentBasedDeduplication": "true"},
	}).Output["TopicArn"])
	for _, queue := range []string{"order-one.fifo", "order-two.fifo"} {
		if _, err := qp.Invoke(ctx, &spi.Request{Identity: id, Operation: "CreateQueue", Input: map[string]any{
			"QueueName": queue, "Attributes": map[string]any{"FifoQueue": "true", "ContentBasedDeduplication": "true"},
		}}); err != nil {
			t.Fatal(err)
		}
		invokeSNS(t, p, id, "Subscribe", map[string]any{"TopicArn": topic, "Protocol": "sqs", "Endpoint": "arn:aws:sqs:us-east-1:1:" + queue, "RawMessageDelivery": "true"})
	}
	for i, body := range []string{"zero", "one", "two"} {
		invokeSNS(t, p, id, "Publish", map[string]any{"TopicArn": topic, "Message": body, "MessageGroupId": "group", "MessageDeduplicationId": fmt.Sprintf("id-%d", i)})
	}
	var expected []string
	for _, queue := range []string{"order-one.fifo", "order-two.fifo"} {
		received := asSlice(invokeSNSQueue(t, qp, id, "ReceiveMessage", map[string]any{"QueueName": queue, "MaxNumberOfMessages": 10, "AttributeNames": []any{"All"}}).Output["Messages"])
		if len(received) != 3 {
			t.Fatalf("queue %s received %#v", queue, received)
		}
		var bodies []string
		for _, message := range received {
			bodies = append(bodies, str(asMap(message)["Body"]))
		}
		if expected == nil {
			expected = bodies
		} else if !reflect.DeepEqual(expected, bodies) {
			t.Fatalf("queue %s order=%v want=%v", queue, bodies, expected)
		}
	}
}
...
func TestSNSFIFONoContentDeduplicationDelivery(t *testing.T) {
	...
	id := spi.Identity{Account: "1", Region: "us-east-1"}
	if _, err := qp.Invoke(ctx, &spi.Request{Identity: id, Operation: "CreateQueue", Input: map[string]any{
		"QueueName": "sns-no-cbd.fifo", "Attributes": map[string]any{"FifoQueue": "true"},
	}}); err != nil {
		t.Fatal(err)
	}
	topic := str(invokeSNS(t, p, id, "CreateTopic", map[string]any{"Name": "sns-no-cbd.fifo", "Attributes": map[string]any{"FifoTopic": "true"}}).Output["TopicArn"])
	invokeSNS(t, p, id, "Subscribe", map[string]any{"TopicArn": topic, "Protocol": "sqs", "Endpoint": "arn:aws:sqs:us-east-1:1:sns-no-cbd.fifo"})
	input := map[string]any{"TopicArn": topic, "Message": "explicit-dedup", "MessageGroupId": "group", "MessageDeduplicationId": "dedup-id"}
	invokeSNS(t, p, id, "Publish", input)
	invokeSNS(t, p, id, "Publish", input)
	received := asSlice(invokeSNSQueue(t, qp, id, "ReceiveMessage", map[string]any{"QueueName": "sns-no-cbd.fifo", "MaxNumberOfMessages": 10}).Output["Messages"])
	if len(received) != 1 || str(asMap(received[0])["Body"]) == "" {
		t.Fatalf("FIFO no-CBD delivery=%#v", received)
	}
}
...
func TestSNSPlatformEndpointLifecycleValidation(t *testing.T) {
	...
	if _, err := call("GetEndpointAttributes", map[string]any{"EndpointArn": endpointARN}); err == nil {
		t.Fatal("found endpoint for deleted application")
	}
...
func TestSNSPlatformApplicationDeletionCleansSubscriptions(t *testing.T) {
	deps := spitest.Deps(t)
	p := New(deps)
	ctx := context.Background()
	id := spi.Identity{Account: "1", Region: "us-east-1"}
	app, err := p.Invoke(ctx, &spi.Request{Identity: id, Operation: "CreatePlatformApplication", Input: map[string]any{
		"Name": "application-delete", "Platform": "GCM", "Attributes": map[string]any{"PlatformCredential": "secret"},
	}})
	if err != nil {
		t.Fatal(err)
	}
	...
	topic, err := p.Invoke(ctx, &spi.Request{Identity: id, Operation: "CreateTopic", Input: map[string]any{"Name": "application-delete"}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := p.Invoke(ctx, &spi.Request{Identity: id, Operation: "Subscribe", Input: map[string]any{
	...
	if _, err := p.Invoke(ctx, &spi.Request{Identity: id, Operation: "DeletePlatformApplication", Input: map[string]any{"PlatformApplicationArn": app.Output["PlatformApplicationArn"]}}); err != nil {
		t.Fatal(err)
	}
	listed, err := p.Invoke(ctx, &spi.Request{Identity: id, Operation: "ListSubscriptionsByTopic", Input: map[string]any{"TopicArn": topic.Output["TopicArn"]}})
	if err != nil {
	...
	if len(asSlice(listed.Output["Subscriptions"])) != 0 {
		t.Fatalf("application subscription remained: %#v", listed.Output)
	}
}
...
func TestSNSSetSubscriptionAttributesAfterUnsubscribe(t *testing.T) {
	...
	for _, malformed := range []string{"randomstring", "randomstring:1", ""} {
		if _, err := p.Invoke(ctx, &spi.Request{Identity: id, Operation: "Unsubscribe", Input: map[string]any{"SubscriptionArn": malformed}}); err == nil {
			t.Fatalf("accepted malformed subscription ARN %q", malformed)
		}
	}
