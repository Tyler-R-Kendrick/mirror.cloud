...
func TestStatesDataFlowValidation(t *testing.T) {
	for _, definition := range []string{
		...
		`{"StartAt":"Pass","States":{"Pass":{"Type":"Pass","OutputPath":"$['\\u00e9']","End":true}}}`,
	...
	for _, definition := range []string{
		...
		`{"StartAt":"Bad","States":{"Bad":{"Type":"Pass","InputPath":"$['\\q']","End":true}}}`,
...
func TestStatesTestStateItemReaderData(t *testing.T) {
	...
	request.Input["stateConfiguration"] = map[string]any{"mapItemReaderData": true}
	if _, err := p.Invoke(context.Background(), request); err == nil {
		t.Fatal("TestState accepted non-string ItemReader data")
	}
	request.Input["stateConfiguration"] = map[string]any{"mapItemReaderData": `{"records":[1,2]}`}
	for _, definition := range []string{
...
func TestStatesLifecycleAndWalkerUnits(t *testing.T) {
	...
	mockedTask := must("TestState", map[string]any{
		"definition": `{"Type":"Task","Resource":"arn:aws:states:::unknown","ResultSelector":{"value.$":"$.answer"},"ResultPath":"$.task","Next":"After"}`,
		"input":      `{"keep":true}`,
